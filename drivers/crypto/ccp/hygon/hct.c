...
struct hct_shared_cfg {
	volatile unsigned int iova_slot[MCCP_IOVA_MAX_SLOT];
	volatile unsigned int ccp_queue_state[MCCP_QUEUES_MAX];
	volatile unsigned int ccps_ref[MCCP_DEV_MAX];
	volatile unsigned int ccps_ref_lock;
	int rsvd1[15];
	volatile u64 qidx[MCCP_QUEUES_MAX];
	volatile unsigned int ccp_state[MCCP_DEV_MAX];
...
static int hct_dev_cmd_queue_init(struct pci_dev *pdev, struct hct_dev_ctx *dev_ctx, int idx)
{
	...
	/* To request_irq, the fourth parameter dev_name must be global variable or static variable. */
