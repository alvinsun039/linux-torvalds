...
DECLARE_WAIT_QUEUE_HEAD(psp_int_queue);
...
static ssize_t read_psp(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	if ((*ppos + count) > PAGE_SIZE) {
		printk(KERN_INFO "%s: invalid address range, pos %llx, count %lx\n",
			__func__, *ppos, count);
		return -EFAULT;
	}
...
int hygon_psp_additional_setup(struct sp_device *sp)
{
	...
	if (!psp_misc) {
		...
		*(u32 *)((void *)psp_misc->data_pg_aligned + 8) = 0xdeadbeef;
...
u8 psp_legacy_rb_supported;	// support legacy ringbuffer
u8 psp_rb_oc_supported;		// support overcommit
u8 psp_generic_rb_supported;	// support generic ringbuffer
...
void psp_ringbuffer_dequeue(struct csv_ringbuffer_queue *ringbuffer,
			    struct csv_cmdptr_entry *cmdptr,
			    struct csv_statval_entry *statval, u32 num)
{
	int i;
	u32 orig_head, que_size;
...
int psp_register_cmd_notifier(u32 cmd_id, p2c_notifier_t notifier)
{
	int ret = -ENODEV;
	unsigned long flags;
...
int psp_unregister_cmd_notifier(u32 cmd_id, p2c_notifier_t notifier)
{
	int ret = -ENODEV;
	unsigned long flags;
	...
	while (status && (count++ < PSP2CPU_MAX_LOOP)) {
		...
		if (status & PSP_X86_CMD) {
			...
			if (!(reg & PSP_CMDRESP_RESP)) {
				...
				p2c_cmd = (u32)(reg & SEV_CMDRESP_IOC);
