...
static int check_gpa_range(struct vpsp_dev_ctx *vpsp_ctx, gpa_t addr, u32 size)
{
	if (!vpsp_ctx || !addr)
		return -EFAULT;
...
static int kvm_bind_vtkm(u32 vm_handle, u32 cmd_id, u32 vid, u32 *pret)
{
	int ret = 0;
 ...
 */
int kvm_pv_psp_forward_op(struct kvm_vpsp *vpsp, u32 cmd,
			  gpa_t data_gpa, u32 psp_ret)
{
	int ret;
	u32 vid = 0;
	...
	ret = vpsp_try_bind_vtkm(vpsp, vpsp_dev_ctx, cmd, (u32 *)&psp_async);
	if (unlikely(ret || *(u32 *)&psp_async)) {
 ...
 */
u32 allow_default_vid = 1;
void vpsp_set_default_vid_permission(u32 is_allow)
{
 ...
 */
static int vpsp_add_vid(u32 vid)
{
...
static void vpsp_ring_update_head(int prio, u32 new_head)
{
	struct csv_ringbuffer_queue *ring_buffer = &vpsp_ring_buffer[prio];
	u32 orig_head = cmd_queue_head(&ring_buffer->cmd_ptr);
	u32 comple_num = 0, index = orig_head;
...
{
	int ret = 0;
	u8 prio = cmd_ctx->rb_prio;
	u16 statval = PSP_CMD_STATUS_RUNNING;
	u32 index = cmd_ctx->rb_index;
		...
		struct vpsp_cmd_ctx *cmd_ctx, struct vpsp_ret *psp_ret)
{
	...
	int rb_supported;
	int index = -1;
	u8 prio = CSV_COMMAND_PRIORITY_LOW;
 ...
 */
int vpsp_parse_ringbuffer_cmd_prio(u8 *prio,
