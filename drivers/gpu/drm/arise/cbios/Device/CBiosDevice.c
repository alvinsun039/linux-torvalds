...
#ifndef __LINUX__
    // if Arise10C0, force to set bSupportHDAudio = 0 in windows
    if((pcbe->ChipID == CHIPID_E3K) || (pcbe->ChipID == CHIPID_ARISE10C0T))
    {
        pMonitorAttribute->bSupportHDAudio = CBIOS_FALSE;
    }
#endif
...
#else
        cbHDCP_OnOff(pcbe, pContentProtectionOnOffParams->DevicesId, IGAIndex, pContentProtectionOnOffParams->bHdcpStatus, hdcpVer);
#endif
        Status = CBIOS_OK;
    }
    else
