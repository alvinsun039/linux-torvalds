...
unsigned long multicorebase1_actual[GB02MAC12];

unsigned long multicorebase1[GB02MAC12] = {
	GB02MAC220,
	GB02MAC221,
	0,
	0
...
unsigned int iosize1[GB02MAC12] = {
	GB02MAC222,
	GB02MAC223,
	0,
	0
};
atomic_t irq_rx1 = ATOMIC_INIT(0);
atomic_t irq_tx1 = ATOMIC_INIT(0);

/* spinlock_t owner_lock = SPIN_LOCK_UNLOCKED; */
DEFINE_SPINLOCK(owner1_lock);
DECLARE_WAIT_QUEUE_HEAD(dec1_wait_queue);
DECLARE_WAIT_QUEUE_HEAD(pp1_wait_queue);
DECLARE_WAIT_QUEUE_HEAD(hw1_queue);

hantrodec_t hantrodec1_data; /* dynamic allocation? */

struct GB02STR31 *dec1_private_data;

unsigned long *GB02FUNC1018(void)
{
	return multicorebase1;
}
...
static void ReleaseIO(void)
{
	...
	for (i = 0; i < hantrodec1_data.cores; i++) {
		for (j = 0; j < HW_CORE_MAX; j++) {
			if (hantrodec1_data.hwregs[i][j]) {
				iounmap((void *) hantrodec1_data.hwregs[i][j]);
				release_mem_region(dec_data->vpu_subsys[i].base_addr
				 + dec_data->vpu_subsys[i].submodule_offset[j],
				  dec_data->vpu_subsys[i].submodule_iosize[j]);
				hantrodec1_data.hwregs[i][j] = 0;
			}
		}
	}
}
...
static void GB02FUNC433(int i, int j)
{
	...
	dec_data = GB02FUNC1137();
	hwid = ioread32((void *)(hantrodec1_data.hwregs[i][j]
	 + GB02MAC461));
	axife_config = ioread32((void *)(hantrodec1_data.hwregs[i][j]));
...
static int GB02FUNC444(int i, int j)
{
	...
	dec_data = GB02FUNC1137();
	if (hantrodec1_data.hwregs[i][j] == NULL) {
	...
	if (dec_data->vpu_subsys[i].has_apbfilter[j]) {
		...
		hwid = ioread32((void *)(hantrodec1_data.hwregs[i][HW_VC8000D]));
		gb_printf(KERN_INFO, "%s-%d: vc800d %d, hwid=%d, hwregsaddr=%pK\n",
		 __func__, __LINE__, HW_VC8000D, hwid,
		  hantrodec1_data.hwregs[i][HW_VC8000D]);

		if (IS_BIGOCEAN(hwid & 0xFFFF)) {
			if (j == HW_AXIFE)
				GB02FUNC1033(i, j, dec_data);
		} else {
			hwid = ioread32((void *)
			(hantrodec1_data.hwregs[i][HW_VC8000D]
				+ GB02MAC461));
		...
		hantrodec1_data.apbfilter_hwregs[i][j] =
			hantrodec1_data.hwregs[i][j] +
			dec_data->GB02STR24[i][j].mask_reg_offset;
	} else
...
static int ReserveIO(void)
{
	...
	memcpy(multicorebase1_actual, multicorebase1,
	 GB02MAC12 * sizeof(unsigned long));
	memcpy((unsigned int *)(hantrodec1_data.iosize),
	 iosize1, GB02MAC12 * sizeof(unsigned int));
	memcpy((unsigned int *)(hantrodec1_data.irq),
	 irq1, GB02MAC12 * sizeof(int));
		...
		for (j = 0; j < HW_CORE_MAX; j++) {
			if (dec_data->vpu_subsys[i].submodule_iosize[j]) {
				...
				gb_printf(KERN_INFO, "%s: decHW %d reg[0] = 0x%08x [%s]\n",
				 __func__, j,
				  readl(hantrodec1_data.hwregs[i][j]),
				   CoreTypeStr(j));
			} else
				hantrodec1_data.hwregs[i][j] = NULL;
		}
		hantrodec1_data.cores++;
	}

	/* check for correct HW */
	if (!CheckHwId(&hantrodec1_data)) {
		ReleaseIO();
		return -1;
	}
	return 0;
...
long Dec1FlushRegs(hantrodec_t *dev, struct GB02STR27 *core)
{
	long ret = 0, i;
	u32 id = core->id;
	u32 type = core->type;
...
long Dec1WriteRegs(hantrodec_t *dev, struct GB02STR27 *core)
{
	long ret = 0;
	u32 i = core->reg_id;
	u32 id = core->id;
...
long Dec1WriteApbFilterRegs(hantrodec_t *dev, struct GB02STR27 *core)
{
	long ret = 0;
	u32 i = core->reg_id;
	u32 id = core->id;
...
long Dec1RefreshRegs(hantrodec_t *dev, struct GB02STR27 *core)
{
	long ret, i;
	u32 id = core->id;
	u32 type = core->type;
...
long Dec1ReadRegs(hantrodec_t *dev, struct GB02STR27 *core)
{
	long ret;
	u32 id = core->id;
	u32 i = core->reg_id;
...
int GetDec1Core(long core, hantrodec_t *dev,
	struct drm_file *filp, unsigned long format)
{
	...
	spin_lock_irqsave(&owner1_lock, flags);
	...
	spin_unlock_irqrestore(&owner1_lock, flags);

	return success;
}

int GetDec1CoreAny(long *core, hantrodec_t *dev,
	struct drm_file *filp, unsigned long format)
{
	int success = 0;
	long c;
	...
	*core = -1;
	for (c = 0; c < dev->cores; c++) {
		...
		if (GetDec1Core(c, dev, filp, format)) {
			success = 1;
			*core = c;
			break;
		}
...
long GB02FUNC1091(hantrodec_t *dev, struct drm_file *filp,
	unsigned long format)
{
	...
	if (wait_event_interruptible(hw1_queue,
		GetDec1CoreAny(&core, dev, filp, format) != 0))
		return -ERESTARTSYS;
...
void ReleaseDecoder1(hantrodec_t *dev, long core)
{
	u32 status;
	unsigned long flags;
	struct GB02STR31 *dec_data = NULL;
	...
	wake_up_interruptible_all(&hw1_queue);
}
...
long WaitDec1ReadyAndRefreshRegs(hantrodec_t *dev, struct GB02STR27 *core)
{
	u32 id = core->id;
	long ret;
	struct GB02STR31 *dec_data = NULL;
	...
	ret = wait_event_interruptible(dec1_wait_queue, CheckDecIrq(dev, id));
	if (ret) {
	...
	/* refresh registers */
	return Dec1RefreshRegs(dev, core);
}
...
long GB02FUNC1103(hantrodec_t *dev, const struct drm_file *filp, int *id)
{
	long ret;
	struct GB02STR31 *dec_data = NULL;
	...
	ret = wait_event_interruptible(dec1_wait_queue,
...
int GB02FUNC1106(hantrodec_t *dev, struct drm_file *filp,
		unsigned long format)
{
	long c;
	unsigned long flags;
...
irqreturn_t hantrodec1_isr(int irq, void *dev_id, struct pt_regs *regs)
#else
irqreturn_t hantrodec1_isr(int irq, void *dev_id)
#endif
{
	unsigned long flags;
	...
	for (i = 0; i < dev->cores; i++) {
		...
		if (irq_status_dec & GB02MAC444) {
			...
			wake_up_interruptible_all(&dec1_wait_queue);
			handled++;
		}
	}
...
long GB02FUNC1109(struct drm_file *filp,
	unsigned int cmd, void *arg)
{
	...
	switch (cmd) {
		case GBDEC_IOCGHWOFFSET: {
			...
			if (id >= hantrodec1_data.cores)
				return -EFAULT;
			__put_user(multicorebase1_actual[id],
			 (unsigned long *) arg);
		break;
	}
	...
	case GBDEC_IOC_MC_OFFSETS: {
		tmp = copy_to_user((unsigned long *) arg,
			multicorebase1_actual, sizeof(multicorebase1_actual));
		if (err) {
	...
	case GBDEC_IOC_MC_CORES:
		__put_user(hantrodec1_data.cores, (unsigned int *) arg);
	...
	case GBDEC_IOCS_DEC_PUSH_REG: {
		...
		return Dec1FlushRegs(&hantrodec1_data, &core);
	}
	case GBDEC_IOCS_DEC_WRITE_REG: {
		...
		return Dec1WriteRegs(&hantrodec1_data, &core);
	}
	...
	struct GB02STR27 core;
		...
		return Dec1WriteApbFilterRegs(&hantrodec1_data, &core);
	}
	case GBDEC_IOCS_DEC_PULL_REG: {
		...
		return Dec1RefreshRegs(&hantrodec1_data, &core);
	}
	case GBDEC_IOCS_DEC_READ_REG: {
		...
		return Dec1ReadRegs(&hantrodec1_data, &core);
	}
	case GBDEC_IOCH_DEC_RESERVE: {
		...
		gb_printf(KERN_INFO, "%s: Reserve DEC core, format = %i\n",
		 __func__, format);
		return GB02FUNC1091(&hantrodec1_data, filp, format);
	}
	case GBDEC_IOCT_DEC_RELEASE: {
		u32 core = 0;

		__get_user(core, (unsigned long *)arg);
		if (core >= hantrodec1_data.cores
			|| dec_data->dec_owner[core] != filp) {
		...
		ReleaseDecoder1(&hantrodec1_data, core);
		break;
	}
	case GBDEC_IOCG_CORE_WAIT: {
		int id;

		tmp = GB02FUNC1103(&hantrodec1_data, filp, &id);
	...
	case GBDEC_IOX_ASIC_ID: {
		...
		if (dec_data->vpu_subsys[core.id].submodule_hwregs[core.type])
			...
			hantrodec1_data.hwregs[core.id][core.type]);
		else if (core.type == HW_VC8000D
			&& hantrodec1_data.hwregs[core.id][HW_VC8000DJ])
			core.asic_id = ioread32((void *)
			hantrodec1_data.hwregs[core.id][HW_VC8000DJ]);
		else
			core.asic_id = 0;
	...
	case GBDEC_IOCG_CORE_ID: {
		...
		return GB02FUNC1106(&hantrodec1_data, filp, format);
	}
	case GBDEC_IOX_ASIC_BUILD_ID: {
		...
		if (id >= hantrodec1_data.cores)
			return -EFAULT;
		if (hantrodec1_data.hwregs[id][HW_VC8000D]
			|| hantrodec1_data.hwregs[id][HW_VC8000DJ]) {
			volatile u8 *hwregs;

			/* VC8000D first if it exists, otherwise VC8000DJ. */
			if (hantrodec1_data.hwregs[id][HW_VC8000D])
				hwregs = hantrodec1_data.hwregs[id][HW_VC8000D];
			else
				hwregs = hantrodec1_data.hwregs[id][HW_VC8000DJ];
		...
		} else if (hantrodec1_data.hwregs[id][HW_BIGOCEAN]) {
			hw_id = ioread32((void *)
			(hantrodec1_data.hwregs[id][HW_BIGOCEAN]));
	...
	case GBDEC_DEBUG_STATUS: {
		...
		for (tmp = 0; tmp < hantrodec1_data.cores; tmp++) {
	...
	default: {
		if (_IOC_TYPE(cmd) == GB02MAC236)
			return (GB02FUNC1424(filp, cmd, arg));
		return -ENOTTY;
	}
	}
			   ...
			   struct GB02STR126 vpu_offset)
{
	...
	dec_data->irq_rx = &irq_rx1;
	dec_data->irq_tx = &irq_tx1;
	dec_data->dec_irq = 0;
	dec_data->pp_irq = 0;
	...
	struct GB02STR20 dec_pcie)
{
	...
	for (i = 0; i < GB02MAC9; i++) {
		if (subsys[i].base_addr == 0x1000)
			...
			subsys[i].base_addr += dec_pcie.vpu_paddr_base;
			multicorebase1[i] += dec_pcie.vpu_paddr_base;
		}
	...
	struct GB02STR3 *vcmd_reg, struct GB02STR126 vpu_offset)
{
	...
	gbdec_data = &hantrodec1_data;

	GB02FUNC1138(dec_data, (struct GB02STR69 *)pci_bars,
	 dev_info, vpu_offset);
	dec1_private_data = dec_data;
	dec_paddr = dec_data->dec_pcie.vpu_paddr_base;
	CheckSubsysCoreArray1(dec_data->vpu_subsys, &vcmd, &total_vcmd_core_num);
	GB02FUNC593(dec_data->vpu_subsys, dec_data->dec_pcie);

	if (base_port != -1) {
		multicorebase1[0] = base_port;
		multicorebase1[0] += GB02MAC220;
		dec_data->vpu_subsys[0].base_addr = base_port;
		gb_printf(KERN_INFO, "hantrodec: Init single core at %lu IRQ = %d\n",
			multicorebase1[0], irq1[0]);
	} else {
		gb_printf(KERN_INFO, "hantrodec: Init multi core[0] at %lu\n"
	       ...
	       "           IRQ_0 = %d\n"
	       "           IRQ_1 = %d\n",
	       multicorebase1[0], multicorebase1[1],
	       multicorebase1[2], multicorebase1[3],
	       irq1[0], irq1[1]);
	}
	hantrodec1_data.cores = 0;
	hantrodec1_data.iosize[0] = GB02MAC222;
	hantrodec1_data.irq[0] = irq1[0];
	hantrodec1_data.iosize[1] = GB02MAC223;
	hantrodec1_data.irq[1] = irq1[1];
	...
	hantrodec1_data.async_queue_dec = NULL;
	hantrodec1_data.async_queue_pp = NULL;
	...
	for (i = 0; i < hantrodec1_data.cores; i++)
		AXIFEEnable(hantrodec1_data.hwregs[i][HW_AXIFE]);

	if (vcmd) {
		result = GB02FUNC1397(dec_data->dec_pcie,
 ...
 * Return type     : int
*/
void GB02FUNC1142(int total_vcmd_core_num)
{
	hantrodec_t *dev = &hantrodec1_data;
	int i;
	...
	GB02FUNC1410(total_vcmd_core_num);
	ReleaseIO();
	vfree(GB02FUNC1137());
