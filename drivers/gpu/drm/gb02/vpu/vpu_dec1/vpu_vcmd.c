...
DECLARE_WAIT_QUEUE_HEAD(vcmd1_cmdbuf_memory_wait);
DEFINE_SPINLOCK(vcmd1_cmdbuf_alloc_lock);
DEFINE_SPINLOCK(vcmd1_process_manager_lock);

/* for all vcmds, the core info should be listed here for subsequent use */
struct GB02STR34 vcmd_core_array1[GB02MAC9] = {
...
static int GB02FUNC702(int total_vcmd_core_num, u64 dec_offset)
{
	...
	for (i = 0; i < total_vcmd_core_num; i++) {
		vcmd_core_array1[i].vcmd_base_addr  = dec_offset
			+ vcmd_core_array1[i].vcmd_base_addr;
	}
...
void GB02FUNC1192(struct GB02STR37  *dev, int total_vcmd_core_num)
{
	int i, n;
	u32 ret;
	...
	struct drm_gb_cmdbuf_resv *input_para)
{
	...
	spin_lock_irqsave(&vcmd1_cmdbuf_alloc_lock, flags);
	if (dec_data->cmdbuf_used_residual == 0) {
		spin_unlock_irqrestore(&vcmd1_cmdbuf_alloc_lock, flags);
		return 0;
	}
...
static void GB02FUNC727(u16 cmdbuf_id)
{
	...
	dec_data->cmdbuf_used_residual += 1;
	spin_unlock_irqrestore(&vcmd1_cmdbuf_alloc_lock, flags);
	wake_up_interruptible_all(&vcmd1_cmdbuf_memory_wait);
}
...
void GB02FUNC1222(struct GB02STR37 *dev,
	bi_list *list, bi_list_node *new_cmdbuf_node,
	struct GB02STR18 *GB02STR18)
{
	int counter = 0;
...
int GB02FUNC1226(struct GB02STR37 *dev,
			       bi_list *list, bi_list_node *new_cmdbuf_node,
			       struct GB02STR18 *GB02STR18)
{
	int counter = 0;
...
static int GB02FUNC773(bi_list_node *new_cmdbuf_node)
{
	...
	GB02FUNC1226(dev, list, new_cmdbuf_node, GB02STR18);
	return 0;
}
	...
	struct drm_gb_cmdbuf_resv *input_para)
{
	...
	if (wait_event_interruptible(vcmd1_cmdbuf_memory_wait,
...
long GB02FUNC1297(struct drm_file *filp,
	...
	bi_list_node **global_cmdbuf_node, struct GB02STR3 vcmd_buff)
{
	...
	spin_lock_irqsave(&vcmd1_process_manager_lock, flags);
	...
	while (1) {
		if (process_manager_node == NULL) {
			...
			spin_unlock_irqrestore(&vcmd1_process_manager_lock,
			 flags);
			return -1;
		}
		GB02STR17 =
	...
	spin_unlock_irqrestore(&vcmd1_process_manager_lock, flags);
...
void GB02FUNC1304(struct GB02STR37 *dev, u32 *jmp_addr,
	 struct GB02STR18 *GB02STR18)
{
	if (dev->hw_version_id > GB02MAC186) {
		jmp_addr = GB02STR18->cmdbuf_virtual_address;
	...
	*(jmp_addr - 4) = opcode_bak | (tempOpcode & 0x2000000);
	...
	if (dev->working_state == GB02MAC145) {
		...
		if (last_cmdbuf_node && last_cmdbuf_node->data) {
			gb_printf(KERN_INFO,
				  "%s:%d---vcmd start for cmdbuf id %d, cmdbuf_run_done = %d\n",
				  __func__, __LINE__,
				  ((struct GB02STR18 *)last_cmdbuf_node->data)->cmdbuf_id,
				  ((struct GB02STR18 *)last_cmdbuf_node->data)->cmdbuf_run_done);
		}
		GB02FUNC1274(dev, last_cmdbuf_node);
	} else {
...
(struct drm_gb_cmdbuf_resv *input_para, bi_list_node **global_cmdbuf_node)
{
	...
	gb_printf(KERN_INFO, "%s:%d----cmdbuf bus addr = %lx, vaddr = %px-\n",
		  __func__, __LINE__, cmdbuf->cmdbuf_bus_address,
		  set_base_addr);
	...
	struct GB02STR37 *hantrovcmd_data)
{
	...
	submodule_main_addr =
	 vcmd_manager[input_para.module_type][0]->vcmd_core_cfg.submodule_main_addr;
	ret = GB02FUNC1297(NULL, &input_para,
	...
	GB02FUNC1308(NULL, input_para.cmdbuf_id,
		input_para.cmdbuf_size, &core_id);
	if (hw_info->hw_id != 0)
		return;
	GB02FUNC1323(NULL, input_para.cmdbuf_id, &irq_status_ret);
...
int GB02FUNC1347(struct drm_file *filp, void **dec_priv, void *data)
{
	int result = 0;
	unsigned long flags;
...
void GB02FUNC1353(struct GB02STR37 *dev,
...
void GB02FUNC1369(struct GB02STR37 *dev,
...
int GB02FUNC1372(struct GB02STR37 *dev,
	...
	struct GB02STR18 *restart_cmdbuf, u32 core_id)
{
	...
	if (cmdbuf_obj_temp->cmdbuf_run_done) {
		...
		GB02FUNC1369(&dev[core_id], new_cmdbuf_node);
...
int GB02FUNC1384(struct drm_file *filp, void *dec_priv)
{
	...
	for (core_id = 0;
	 core_id < dec_data->total_vcmd_core_num; core_id++) {
		if ((&dev[core_id]) == NULL)
			continue;
		...
		while (1) {
			...
			if (dev[core_id].hwregs
			 && (cmdbuf_obj_temp->filp == filp)) {
				vcmd_aborted =
				 GB02FUNC1372(dev, new_cmdbuf_node,
		...
		GB02FUNC1353(dev, restart_cmdbuf, core_id);
...
{
	irqreturn_t ret = 0;
	ret = hantrovcmd_d1isr(irq, (void *)&hantrovcmd_data[0]);
	return ret;
}
int GB02FUNC1392(int irq_id)
{
	...
	for (i = 0; i < dec_data->total_vcmd_core_num; i++) {
		...
		if (hantrovcmd_data[i].vcmd_core_cfg.vcmd_irq != -1) {
			ret = request_irq(irq_id, hantrovcmd_d1isr,
	...
	u64 dec_offset)
{
	...
	for (i = 0; i < total_vcmd_core_num; i++)
		gb_printf(KERN_INFO, "%s: module init - vcmdcore[%d] addr =0x%lx\n",
		 __func__, i, vcmd_core_array1[i].vcmd_base_addr);
	hantrovcmd_data =
	...
	GB02FUNC1395(dec_data, dec_pcie, vcmd_pool, vcmd_reg);
	GB02FUNC208(&dec_data->global_process_manager);
	for (i = 0; i < total_vcmd_core_num; i++) {
		hantrovcmd_data[i].vcmd_core_cfg = vcmd_core_array1[i];
		hantrovcmd_data[i].hwregs = NULL;
		...
		vcmd_manager[vcmd_core_array1[i].sub_module_type][dec_data->vcmd_type_core_num[vcmd_core_array1[i].sub_module_type]] =
		&hantrovcmd_data[i];
		dec_data->vcmd_type_core_num[vcmd_core_array1[i].sub_module_type]++;
	...
	result = GB02FUNC1195(hantrovcmd_data, total_vcmd_core_num,
	 (u8 *)dec_pcie.vpu_vaddr_base);
	if (result < 0)
		goto err;
	GB02FUNC1192(hantrovcmd_data, total_vcmd_core_num);
...
void GB02FUNC1404(void)
{
	...
	gb_printf(KERN_INFO, "%s:%d, dec1 resume in\n", __func__, __LINE__);
	...
	GB02FUNC1192(hantrovcmd_data, 1);

	gb_printf(KERN_INFO, "%s:%d, dec1 resume out\n", __func__, __LINE__);
}

void GB02FUNC1405(void)
{
	gb_printf(KERN_INFO, "%s:%d, dec1 suspend in\n", __func__, __LINE__);
...
void GB02FUNC1410(int total_vcmd_core_num)
{
	...
	GB02FUNC1194(hantrovcmd_data, total_vcmd_core_num);
	vfree(hantrovcmd_data);
...
int GB02FUNC1421(struct drm_file *filp, void *data)
{
	...
	ret = GB02FUNC1297(filp, input_para,
		&dec_data->global_process_manager,
