...
static void GB02FUNC612(struct GB02STR37 *dev)
{
	...
	GB02FUNC705(dev, base_cmdbuf_node);
	GB02FUNC793(dev, base_cmdbuf_node);
	if (dev->sw_cmdbuf_rdy_num != 0)
		GB02FUNC813(dev, base_cmdbuf_node);
}
...
irqreturn_t hantrovcmd_disr(int irq, void *dev_id, struct pt_regs *regs)
#else
irqreturn_t hantrovcmd_disr(int irq, void *dev_id)
#endif
{
