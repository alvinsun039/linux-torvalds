...
extern struct GB02STR34 vcmd_core_array[GB02MAC9];
/* List of subsystems */
...
static void GB02FUNC684(struct GB02STR23 *subsys,
	int vcmd, int total_vcmd_core_num, int num)
{
	int i;
	unsigned long *multicorebase = GB02FUNC409();
	int *irq = GB02FUNC411();
	unsigned int *iosize = GB02FUNC414();
	...
	if (vcmd) {
		for (i = 0; i < total_vcmd_core_num; i++) {
			...
			vcmd_core_array[i].sub_module_type = 2;
...
void CheckSubsysCoreArray(struct GB02STR23 *subsys, int *vcmd,
	int *total_vcmd_core_num)
{
			...
			if (subsys[core_array[i].subsys].irq
			 != core_array[i].irq) {
				gb_printf(KERN_INFO, "%s: core type: %d, irq %d != sub irq %d\n",
				 __func__, core_array[i].core_type,
				  core_array[i].irq,
	...
	*total_vcmd_core_num = tmp_core_num;
	...
	GB02FUNC684(subsys, *vcmd, *total_vcmd_core_num, num);
