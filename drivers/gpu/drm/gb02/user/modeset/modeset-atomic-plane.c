...
static int modeset_create_fb(int fd, struct buffer_object *bo)
{
	...
	for (i = 0; i < buf.height; i++) {
		for (j = 0; j < buf.width; j++) {
			int offset = bo->pitch * i + j * 4;
			*(uint32_t*)&bo->vaddr[offset] = (0xff << 24) | (r << 16) | (g << 8) | b;
		}
	}
	return 0;
}
...
*/
	...
	drmModeAtomicAddProperty(req, plane_id, property_crtc_w, 0);
	drmModeAtomicAddProperty(req, plane_id, property_crtc_h, 0);
	drmModeAtomicAddProperty(req, plane_id, property_src_x, 0);
	drmModeAtomicAddProperty(req, plane_id, property_src_y, 0);
	drmModeAtomicAddProperty(req, plane_id, property_src_w, 0 << 16);
	drmModeAtomicAddProperty(req, plane_id, property_src_h, 0 << 16);
