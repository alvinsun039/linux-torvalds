...
#define	GB02MAC297
#ifndef	GB02MAC296
#define	GB02MAC298	(2 * 1024)
#define	GB02MAC299	0x50000//0x500000
#define	GB02MAC300	0x10000//0x100000
#define	GB02MAC301	0x100000//0x100000
#define		GB02MAC302  0xc0000000
#define		GB02MAC303	0x1000000

#else

#define	GB02MAC299	0x800//0x500000
#define	GB02MAC304	GB02MAC1043//0x80400000
#define	GB02MAC305	GB02MAC1052//0x81000000
#define	GB02MAC306	0x800000000
#define	GB02MAC307	0x80000000
#endif

#define GB02MAC309	(6)
#define GB02MAC310 (1)
#define GB02MAC312 (2) /* DWAXIDMAC_TRANS_WIDTH_256 */
#define GB02MAC314 (255) /* DWAXIDMAC_ARWLEN_256 */
#define GB02MAC316 (0) /* channel 0 */
#define GB02MAC318 (1)

#define GB02MAC320				(0x1000) /* 4KB */
#define GB02MAC322			(0x400000) /* 4MB */
#define GB02MAC24				(0x4000) /* 16KB */
#define GB02MAC325			(0x100000)
#define GB02MAC327			(0x500000)

#define GB02MAC329 (0x40)

#define GB02MAC332 (0xc0000000) /* view at V2VDMA side */

/* LLI == Linked List Item */
struct __packed GB02STR36 {
	__le64	sar;
	__le64	dar;
	__le32	block_ts_lo;
	__le32	block_ts_hi;
...
struct GB02STR42 {
	...
	int chan_count;
	unsigned __iomem *dma_src_addr;
	unsigned __iomem *dma_dst_addr;

	u64	dma_src_paddr[GB02MAC309];
	u64	dma_dst_paddr;
	int	tsfd_num;
...
static inline u32 GB02FUNC170(struct GB02STR42 *chip, u32 offset)
{
	u32 val = 0;
...
static inline void GB02FUNC173(struct GB02STR42 *chip,
	u32 offset, u32 val)
{
	iowrite32(val, chip->regs + offset);
}

static inline u32 GB02FUNC13(struct GB02STR40 *chan, u32 offset)
{
	u32 val = 0;
...
enum {
	DWAXIDMAC_CH_CTL_SHADOW_LL_INVALID	= 0,
	DWAXIDMAC_CH_CTL_SHADOW_LL_VALID
};
...
struct GB02STR38 *GB02FUNC221(struct GB02STR42 *dma_ctrl,
	size_t len);
void
gb02_dma_multi_blk_linklist_mode_start(struct GB02STR42 *dma_ctrl);
void GB02FUNC114(struct GB02STR42 *chip);
int GB02FUNC147(struct GB02STR42 *dma_ctrl,
	int data_bytes, int ch_id);
