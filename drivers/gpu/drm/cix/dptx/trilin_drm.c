				 ...
				 struct drm_atomic_state *state)
{
	...
	if (dp->mst_mgr != NULL) {
		ret =
		    drm_dp_mst_root_conn_atomic_check(new_con_state,
						      dp->mst_mgr);
		if (ret < 0)
			return ret;
	}
	...
	if (!drm_connector_atomic_hdr_metadata_equal
	    (old_con_state, new_con_state)) {
		if (new_con_state->hdr_output_metadata) {
			ret = drm_hdmi_infoframe_set_hdr_metadata(
					&trilin_conn->drm_infoframe,
					new_con_state);
			if (ret) {
	...
	if (dp->platform_id != CIX_PLATFORM_EMU) {
		...
		if (!edid) {
			...
			if (ret == 0) {
				...
				DP_INFO
				    ("edid is null and read downstream: count=%d",
				     ret);
			}
			return ret;
		}
...
static struct drm_encoder *trilin_dp_connector_best_encoder(struct drm_connector
							    *connector)
{
	struct trilin_dp *dp = connector_to_dp(connector);
...
enum drm_mode_status trilin_dp_connector_mode_valid(struct drm_connector
						    *connector,
						    ...
						    *mode)
{
	...
	if (clock > rate) {
		DP_INFO("filtered mode (%s@%d) for high bandwidth\n",
			mode->name, clock);
		drm_mode_debug_printmodeline(mode);
		return MODE_CLOCK_HIGH;
	}
...
static const struct drm_connector_helper_funcs trilin_dp_connector_helper_funcs = {
...
u8 trilin_dp_cal_bpc(struct drm_connector_state *connector_state)
{
	...
	if ((connector_state->colorspace >= DRM_MODE_COLORIMETRY_BT2020_CYCC) &&
		...
		pr_info("colorspace is bt2020. force set bpc (%d) to 10",
			info->bpc);
		bpc = 10;
	}
...
u8 trilin_dp_cal_bpp(struct drm_connector_state *connector_state, u8 bpc,
				   ...
				   struct drm_connector_state *connector_state)
{
	...
	struct trilin_connector *conn =
	    connector_to_trilin(connector_state->connector);
	struct trilin_dp *dp = encoder_to_dp(encoder);
	...
	DP_DEBUG
	    ("Note: force bus format %d to RGB 0. disp->bpc(%d) bpc(%d) bpp(%d) colorspace=%d\n",
	...
	if (mode->clock > rate) {
		dev_err(dp->dev,
			"mode %s pixel rate %d is higher than max rate %d\n",
...
int trilin_dp_drm_init(struct trilin_dpsub *dpsub)
{
	...
	drm_encoder_init(dp->drm,
			 encoder,
			 &trilin_dp_enc_funcs, DRM_MODE_ENCODER_TMDS, NULL);
