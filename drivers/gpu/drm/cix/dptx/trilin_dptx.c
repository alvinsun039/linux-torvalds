...
void trilin_dp_dump_regs(struct seq_file *m, struct trilin_dp *dp)
{
	...
	if (dp->mst_mgr != NULL) {
		for (j = 1; j < dp->max_streams; j++) {
			...
			for (i = TRILIN_DPTX_VIDEO_STREAM_ENABLE + offset;
					i <= TRILIN_DPTX_SRC0_FRAMING_STATUS + offset; i += 4)
				DUMP_DP_REG(i);
			seq_printf(m, "----Secondary channel, source %d----\n", j);
			for (i = TRILIN_DPTX_SEC0_AUDIO_ENABLE + offset;
					i <= TRILIN_DPTX_SEC0_ADAPTIVE_SYNC_ENABLE + offset; i += 4)
				DUMP_DP_REG(i);
		}
	}
}
