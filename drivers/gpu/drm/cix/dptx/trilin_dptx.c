...
static int trilin_dp_link_train_ce(struct trilin_dp *dp)
{
	...
	if (dp->caps.tps4_supported) {
		...
		ret =
		    drm_dp_dpcd_writeb(&dp->aux, DP_TRAINING_PATTERN_SET, pat);
	} else {
				      ...
				      struct drm_display_mode *mode, int bpp)
{
	...
	if (dp->pixelPerClk == 2 && dp->mode.lane_cnt == 4
	    && dp->mode.link_rate == 810000) {
		data_control = 0x4;
	}

	DP_DEBUG("%s adjust_mode flags: 0x%0x lane_cnt=%d mode.lans=%d (htotal=%d vtotal=%d) data_control=0x%0x",
	     __func__, mode->flags, lane_cnt, dp->mode.lane_cnt, mode->htotal,
	...
	sec_data_window =
	    (mode->htotal - mode->hdisplay) * (dp->mode.link_rate / 40)
	    * 9 / mode->clock;
	...
	if (dp->pixelPerClk == 2)
		trilin_dp_write(dp,
				TRILIN_DPTX_SRC0_USER_PIXEL_COUNT + regs_off,
				2);
}
					   ...
					   uint32_t slot_count)
{
	...
	for (uint32_t i = 0; i < slot_count; i++)
		trilin_dp_write(dp, TRILIN_DPTX_MST_PID_TABLE_ENTRY, 1 << source_id);	// clear
}
...
static __maybe_unused int trilin_dp_panel_setup_colorimetry_sdp(struct trilin_dp
								*dp,
								struct
								trilin_dp_panel
								*dp_panel)
{
	...
	if (conn->config.override & TRILIN_DPTX_STREAM_OVERRIDE_ENABLE) {
		...
		cix_infoframe_write_packet(dp, dp_panel->stream_id,
					   CIX_VSC_SDP, &conn->sdp[CIX_VSC_SDP],
					   1);
				     ...
				     struct trilin_dp_panel *dp_panel)
{
	...
	DP_INFO
	    ("trilin_dp_encoder_disable stream_id=%d, active_stream_cnt=%d\n",
	     dp_panel->stream_id, dp->active_stream_cnt);
...
static int trilin_dp_core_on(struct trilin_dp *dp, bool shallow)
{
	...
	if (rc) {
		...
		trilin_dp_write(dp, TRILIN_DPTX_FORCE_SCRAMBLER_RESET,
				enable_sources);
...
static int trinlin_dp_panel_read_sink_caps(struct trilin_dp *dp)
{
	...
	rlen = drm_dp_dpcd_read(drm_aux,
				DP_DPRX_FEATURE_ENUMERATION_LIST, &rx_feature,
				1);
	if (rlen != 1) {
		...
		dp->caps.vscext_chaining_supported =
		    !!(rx_feature & DP_VSC_EXT_VESA_SDP_CHAINING_SUPPORTED)
	...
	DP_INFO
	    ("mst cap %d rlen=%d max_lanes=%d max_rate=%d sink_count=%d tps3_supported=%d, tps4_supported=%d\n",
	     dp->caps.mst, rlen, config->max_lanes, config->max_rate,
	     dp->sink_count, dp->caps.tps3_supported, dp->caps.tps4_supported);
	return rc;
}
...
static int trilin_dp_link_hdcp_request(struct trilin_dp *dp)
{
	...
	if (status[1] & DP_CP_IRQ) {
		ret =
		    drm_dp_dpcd_read(&dp->aux, DP_HDCP_2_2_REG_RXSTATUS_OFFSET,
				     status, 1);
		if (ret < 0)
			return ret;
...
int trilin_dp_host_init(struct trilin_dp *dp)
{
	...
	if (!phy->phy_ops && dp->platform_id == CIX_PLATFORM_SOC) {
		if (has_acpi_companion(dp->dev)) {
			fwnode =
			    fwnode_find_reference(dp->dev->fwnode, "dp_phy", 0);
			if (!IS_ERR(fwnode)) {
				phy->base =
				    devm_phy_optional_get(to_acpi_device_node
							  (fwnode)->dev.parent,
							  fwnode_get_name
							  (fwnode));
			} else {
...
{
	DP_DEBUG("%s: hpd=%d state=%d sink count:%d is_sink_count_zero=%d\n",
	     __func__, trilin_dp_get_hpd_state(dp), (dp->state & DP_STATE_CONNECTED),
...
void trilin_dp_dump_regs(struct seq_file *m, struct trilin_dp *dp)
{
	...
	seq_puts(m, "----misc control(scrambler_reset/core_features/core_revision)----\n");
	...
	if (dp->mst_mgr != NULL) {
		...
		for (i = TRILIN_DPTX_MST_ENABLE; i <= TRILIN_DPTX_MSO_CONFIG;
			...
			if (i == TRILIN_DPTX_MST_PID_TABLE_ENTRY) {
				trilin_dp_write(dp,
						TRILIN_DPTX_MST_PID_TABLE_INDEX,
						0);
				for (j = 0; j < 64; j++) {
					pidtables[j * 2] =
	...
	if (dp->mst_mgr != NULL) {
		seq_puts(m, "----Main stream control, virtual source 1----\n");
		for (i = TRILIN_DPTX_SRC1_VIDEO_STREAM_ENABLE;
		     i <= TRILIN_DPTX_SRC1_FRAMING_STATUS; i += 4)
			DUMP_DP_REG(i);
		seq_puts(m, "----Secondary channel, source 1----\n");
...
int trilin_dp_probe(struct trilin_dpsub *dpsub, struct drm_device *drm)
{
	...
	if (has_acpi_companion(dev)) {
		...
		res_dp =
		    platform_get_resource_byname(pdev, IORESOURCE_MEM, "dp");
		res_phy =
