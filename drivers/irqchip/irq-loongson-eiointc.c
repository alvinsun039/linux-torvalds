...
struct extioi_node_map {
	DECLARE_BITMAP(bits, MAX_EIO_NODES);
};
...
struct extioi_node_map extioi_node_maps = EXTIOI_NODE_MASK_NONE;
...
static void eiointc_set_irq_route(int pos, unsigned int cpu, unsigned int mnode, nodemask_t *node_map)
{
	...
	for_each_online_cpu(i) {
		...
		if (node_isset(node, eio_node_map)) {
			...
			csr_any_send(EIOINTC_REG_ROUTE + pos_off,
						data, data_mask,
						cpu_logical_map(i));
		}
	}
