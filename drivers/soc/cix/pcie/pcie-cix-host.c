...
void __iomem *cix_cdns_pci_map_bus(struct pci_bus *bus, unsigned int devfn,
			       int where)
{
...
static int cdns_pcie_host_init_address_translation(struct cdns_pcie_rc *rc)
{
	...
	r = 1;
	cix_cdns_pcie_set_outbound_region_for_normal_msg(pcie, busnr, 0, r, pcie->msg_res->start);
	...
	resource_list_for_each_entry(entry, &bridge->windows) {
		...
		if (resource_type(res) == IORESOURCE_IO)
			cix_cdns_pcie_set_outbound_region(pcie, busnr, 0, r,
						      true,
...
int cix_cdns_pcie_host_init(struct cdns_pcie_rc *rc)
{
	int err;
...
int cix_cdns_pcie_host_link_setup(struct cdns_pcie_rc *rc)
{
	struct cdns_pcie *pcie = &rc->pcie;
	...
	if (rc->quirk_detect_quiet_flag)
		cix_cdns_pcie_detect_quiet_min_delay_set(&rc->pcie);
...
int cix_cdns_pcie_host_setup(struct cdns_pcie_rc *rc)
{
	struct device *dev = rc->pcie.dev;
	...
	ret = cix_cdns_pcie_host_link_setup(rc);
	if (ret)
		return ret;
	...
	ret = cix_cdns_pcie_host_init(rc);
	if (ret)
		return ret;
