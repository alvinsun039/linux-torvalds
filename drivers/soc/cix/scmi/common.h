...
extern const struct scmi_desc cix_scmi_mailbox_desc;
#endif
#ifdef CONFIG_ARM_SCMI_TRANSPORT_SMC
extern const struct scmi_desc cix_scmi_smc_desc;
#endif
#ifdef CONFIG_ARM_SCMI_TRANSPORT_VIRTIO
extern const struct scmi_desc cix_scmi_virtio_desc;
#endif
#ifdef CONFIG_ARM_SCMI_TRANSPORT_OPTEE
extern const struct scmi_desc cix_scmi_optee_desc;
#endif
...
void cix_shmem_tx_prepare(struct scmi_shared_mem __iomem *shmem,
		      struct scmi_xfer *xfer, struct scmi_chan_info *cinfo);
u32 shmem_read_header(struct scmi_shared_mem __iomem *shmem);
void cix_shmem_fetch_response(struct scmi_shared_mem __iomem *shmem,
			  struct scmi_xfer *xfer);
void cix_shmem_fetch_notification(struct scmi_shared_mem __iomem *shmem,
			      size_t max_len, struct scmi_xfer *xfer);
...
int cix_scmi_protocol_register(const struct scmi_protocol *proto);
void cix_scmi_protocol_unregister(const struct scmi_protocol *proto);
...
#define DEFINE_CIX_SCMI_PROTOCOL_REGISTER_UNREGISTER(name, proto)	\
static const struct scmi_protocol *__this_proto = &(proto);	\
								\
int __init scmi_##name##_register(void)				\
{								\
	return cix_scmi_protocol_register(__this_proto);		\
}								\
								\
void __exit scmi_##name##_unregister(void)			\
{								\
	cix_scmi_protocol_unregister(__this_proto);			\
}

#define DECLARE_CIX_SCMI_REGISTER_UNREGISTER(func)		\
	int __init scmi_##func##_register(void);	\
	void __exit scmi_##func##_unregister(void)

DECLARE_CIX_SCMI_REGISTER_UNREGISTER(cix_base);
DECLARE_CIX_SCMI_REGISTER_UNREGISTER(cix_clock);
DECLARE_CIX_SCMI_REGISTER_UNREGISTER(cix_perf);
