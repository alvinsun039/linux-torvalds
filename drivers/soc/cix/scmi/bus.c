...
void cix_scmi_device_destroy(struct device *parent, int protocol, const char *name)
{
...
EXPORT_SYMBOL_GPL(cix_scmi_device_destroy);
...
int cix_scmi_driver_register(struct scmi_driver *driver, struct module *owner,
			 const char *mod_name)
{
	int retval;
...
EXPORT_SYMBOL_GPL(cix_scmi_driver_register);
 ...
 */
struct scmi_device *cix_scmi_device_create(struct fwnode_handle *np,
...
EXPORT_SYMBOL_GPL(cix_scmi_device_create);
