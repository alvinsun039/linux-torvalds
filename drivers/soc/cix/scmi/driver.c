...
EXPORT_SYMBOL_GPL(cix_scmi_protocol_register);
...
EXPORT_SYMBOL_GPL(cix_scmi_protocol_unregister);
					 ...
					 int prot_id, const char *name)
{
	...
	sdev = cix_scmi_device_create(np, info->dev, prot_id, name);
	if (name && !sdev)
		dev_err(info->dev,
...
{
	mutex_lock(&info->devreq_mtx);
	cix_scmi_device_destroy(info->dev, prot_id, name);
	mutex_unlock(&info->devreq_mtx);
}
...
/**
 * cix_scmi_protocol_acquire  - Protocol acquire
 * @handle: A reference to the SCMI platform instance.
 * @protocol_id: The protocol being requested.
 *
...
static void scmi_devm_release_protocol(struct device *dev, void *res)
{
	...
	cix_scmi_protocol_release(dres->handle, dres->protocol_id);
}
			   ...
			   int prot_id, bool tx)
{
	...
	tdev = cix_scmi_device_create(fwnode, info->dev, prot_id, name);
	if (!tdev) {
		dev_err(info->dev,
...
static int scmi_chan_destroy(int id, void *p, void *idr)
{
	...
	if (cinfo->dev) {
		...
		cix_scmi_device_destroy(info->dev, id, sdev->name);
		cinfo->dev = NULL;
	}
...
static int scmi_probe(struct platform_device *pdev)
{
	 ...
	 */
	ret = cix_scmi_protocol_acquire(handle, SCMI_PROTOCOL_BASE);
	if (ret) {
...
static const struct acpi_device_id scmi_acpi_match[] = {
	...
	{ .id = "CIXHA006", .driver_data = (kernel_ulong_t)&cix_scmi_mailbox_desc },
#endif
	{ /* Sentinel */ },
};
...
static struct dentry *scmi_debugfs_init(void)
{
	...
	d = debugfs_create_dir("cix-acpi-scmi", NULL);
	if (IS_ERR(d)) {
...
static int __init scmi_driver_init(void)
{
	...
	scmi_cix_base_register();
	scmi_cix_clock_register();
	scmi_cix_perf_register();

	return platform_driver_register(&scmi_driver);
}
//core_initcall(scmi_driver_init);
subsys_initcall_sync(scmi_driver_init);
//module_init(scmi_driver_init);

static void __exit scmi_driver_exit(void)
{
	scmi_cix_base_unregister();
	scmi_cix_clock_unregister();
	scmi_cix_perf_unregister();
