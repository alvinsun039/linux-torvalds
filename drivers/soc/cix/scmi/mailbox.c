...
static void tx_prepare(struct mbox_client *cl, void *m)
{
	...
	cix_shmem_tx_prepare(smbox->shmem, m, smbox->cinfo);
}
			      ...
			      bool tx)
{
	...
	smbox->chan = cix_mbox_request_channel(cl, tx ? 0 : p2a_chan);
	...
	if (tx && a2p_rx_chan) {
		smbox->chan_receiver = cix_mbox_request_channel(cl, a2p_rx_chan);
				   ...
				   struct scmi_xfer *xfer)
{
	...
	cix_shmem_fetch_response(smbox->shmem, xfer);
}
				       ...
				       size_t max_len, struct scmi_xfer *xfer)
{
	...
	cix_shmem_fetch_notification(smbox->shmem, max_len, xfer);
}
...
const struct scmi_desc cix_scmi_mailbox_desc = {
