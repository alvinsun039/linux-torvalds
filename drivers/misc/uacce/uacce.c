				   ...
				   struct vm_area_struct *vma)
{
	...
	unsigned long max_size = PAGE_SIZE << (MAX_PAGE_ORDER - 1);
	struct device *pdev = q->uacce->parent;
	struct uacce_device *uacce = q->uacce;
	...
	ret = uacce_sort_dma_buffers(slice, 0, slice[0].total_num - 1, &slice[ss_num]);
	if (ret) {
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	distance = node_distance(uacce->parent->numa_node, cpu_to_node(smp_processor_id()));
#endif
	return sysfs_emit(buf, "%d\n", distance);
}
