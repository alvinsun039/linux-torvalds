...
static ssize_t rnp_dbg_mbx_cookies_info_read(struct file *filp,
					     char __user *buffer,
					     size_t count, loff_t *ppos)
{
	struct rnp_adapter *adapter = filp->private_data;
	...
	buf = kasprintf(GFP_KERNEL, "pool items:cur:%d total: %d."
			"free:%d wait_free:%d alloced:%d\n",
 ...
 * @ppos: file position offset
 **/
static ssize_t rnp_dbg_command_write(struct file *filp,
...
#else
			pfile = debugfs_create_file("csl", 0755,
#endif
				adapter->rnp_dbg_adapter,
