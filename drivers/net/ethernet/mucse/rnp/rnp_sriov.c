...
#if defined(HAVE_VF_SPOOFCHK_CONFIGURE)
...
#endif
{
	...
	if (adapter->vfinfo[vf].get_mtu_done != true)
		return 0;
	/* send mbx to vf */
			    ...
			    int vf, bool setting)
{
	...
	if (!(adapter->flags & RNP_FLAG_VF_INIT_DONE)) {
		dev_err(pci_dev_to_dev(adapter->pdev),
			"set vf spoofchk failed, vf %d "
			"has not has not been initialized yet, please retry\n",
			vf);
		return -EINVAL;
	}
...
#endif /* HAVE_NDO_SET_VF_MIN_MAX_TX_RATE */
{
	...
	real_rate = (ring_max_rate * 1024 * 128) * 90 / 100;
			  ...
			  struct ifla_vf_info *ivi)
{
	...
	if (!(adapter->flags & RNP_FLAG_VF_INIT_DONE)) {
		dev_err(pci_dev_to_dev(adapter->pdev),
			"get vf config failed, vf %d has not has "
			"not been initialized yet, please retry\n",
			vf);
		return -EINVAL;
	}
	...
	if (vf >= adapter->num_vfs)
		return -EINVAL;
	ivi->vf = vf;
	memcpy(&ivi->mac, adapter->vfinfo[vf].vf_mac_addresses,
			ETH_ALEN);
