...
static bool rnpgbevf_alloc_mapped_skb(struct rnpgbevf_ring *rx_ring,
				      struct rnpgbevf_rx_buffer *bi);
#else /* CONFIG_RNPGBE_DISABLE_PACKET_SPLIT */
				      ...
				      u16 cleaned_count)
{
	...
	do {
#ifdef CONFIG_RNPGBE_DISABLE_PACKET_SPLIT
		if (!rnpgbevf_alloc_mapped_skb(rx_ring, bi))
			break;
#else
...
/* try to match adapter->vf_vlan with skb->data */
static void rnpgbevf_match_vlan(struct sk_buff *skb, struct rnpgbevf_adapter *adapter)
{
	u16 vid_pf;
...
/* try to match desc vlan */
static int rnpgbevf_match_desc(union rnp_rx_desc *rx_desc,
			       struct rnpgbevf_ring *rx_ring,
			       struct sk_buff *skb,
			       struct rnpgbevf_adapter *adapter)
{
	if (rnpgbevf_test_staterr(rx_desc, RNPGBE_RXD_STAT_VLAN_VALID) &&
					...
					struct sk_buff *skb)
{
	...
	if (hw->pf_feature & PF_NCSI_EN) {
		...
		if (adapter->flags & RNPVF_FLAG_PF_SET_VLAN)
			rnpgbevf_match_vlan(skb, adapter);

		goto skip_vf_vlan;
		/* if pf not setup vlan, nothing todo */
		/* ncsi must tx-vlan off, rx-vlan off */
	} else if (adapter->flags & RNPVF_FLAG_PF_SET_VLAN) {
		/* if not ncsi, but pf set a vlan */
		/* tx-vlan off, with rx-vlan off */
		/* first try to match desc */
		/* if desc has vlan, not need match skb->data */
		/* packets from pf or other vf (not pf set vlan) */
		if (rnpgbevf_match_desc(rx_desc, rx_ring, skb, adapter))
			goto skip_vf_vlan;

		/* packets from outer or other vf (pf set vlan) */
		/* second try to match skb->data */
		rnpgbevf_match_vlan(skb, adapter);
		
		goto skip_vf_vlan;
	}
	/* if not ncsi, and pf not set a vlan */
	/* tx-vlan on, with rx-vlan on */
...
static bool rnpgbevf_check_src_mac(struct sk_buff *skb,
				     ...
				     struct sk_buff *skb)
{
	...
	if (!(rx_ring->ring_flags & RNPVF_RING_VEB_MULTI_FIX))
		return rnpgbevf_check_src_mac(skb, rx_ring->netdev);
	else
		return false;
#endif
				...
				union rnp_rx_desc *rx_desc, struct sk_buff *skb)
{
	...
	if (likely(rnpgbevf_test_staterr(rx_desc, RNPGBE_RXD_STAT_EOP)))
		return false;
#ifdef CONFIG_RNPGBE_RNPGBE_DISABLE_PACKET_SPLIT
	printk(KERN_DEBUG "error spilt detect in disable split mode\n");
#else
	/* place skb in next buffer to be received */
				 ...
				 struct rnpgbevf_ring *rx_ring, int budget)
{
	...
	while (likely(total_rx_packets < budget)) {
		...
		if (rnpgbevf_check_csum_error(rx_ring, rx_desc, len,
					      &driver_drop_packets)) {
			dev_kfree_skb_any(skb);
			cleaned_count++;
			err_packets++;
...
rnpgbevf_irq_enable_queues(struct rnpgbevf_q_vector *q_vector)
{
	...
	rnpgbevf_for_each_ring(ring, q_vector->rx) {
		...
		/* we need this */
		wmb();
#ifdef CONFIG_RNPGBE_DISABLE_TX_IRQ
...
#ifdef CONFIG_RNPGBE_DCA
	if (adapter->flags & RNPGBE_FLAG_DCA_ENABLED)
		rnpgbevf_update_dca(q_vector);
#endif
...
int rx_poll_thread_handler(void *data)
{
	...
	printk(KERN_DEBUG "%s  %s running...\n", __func__, adapter->name);

	do {
		for (i = 0; i < adapter->num_q_vectors; i++)
			rnpgbevf_msix_clean_rings(0, adapter->q_vector[i]);

		msleep(20);
	} while (!kthread_should_stop() && adapter->quit_poll_thread != true);

	printk(KERN_DEBUG "%s  %s stoped\n", __func__, adapter->name);
...
static int rnpgbevf_request_irq(struct rnpgbevf_adapter *adapter)
{
	...
	if (!adapter->rx_poll_thread) {
		rnpgbevf_err("kthread_run faild!\n");
		return -EIO;
	}
	return 0;
#endif
...
#else
		ring_wr32(ring, PCI_DMA_REG_RX_SCATTER_LENGH,
...
static int rnpgbevf_vlan_rx_add_vid(struct net_device *netdev, u16 vid)
#endif /* NETIF_F_HW_VLAN_CTAG_TX */
#else /* !HAVE_INT_NDO_VLAN_RX_ADD_VID */
static void rnpgbevf_vlan_rx_add_vid(struct net_device *netdev, u16 vid)
...
#else
		ncsi_vlan_rx_add_vid(netdev, vid);
#endif
	}
#endif
...
static int rnpgbevf_vlan_rx_kill_vid(struct net_device *netdev, u16 vid)
#endif /* NETIF_F_HW_VLAN_CTAG_RX */
#else
static void rnpgbevf_vlan_rx_kill_vid(struct net_device *netdev, u16 vid)
#endif
{
	struct rnpgbevf_adapter *adapter = netdev_priv(netdev);
	struct rnpgbevf_hw *hw = &adapter->hw;
...
#else
		ncsi_vlan_rx_kill_vid(netdev, vid);
#endif
	}
...
static void rnpgbevf_restore_vlan(struct rnpgbevf_adapter *adapter)
{
...
#else /* !NETIF_F_HW_VLAN_CTAG_TX */
	rnpgbevf_vlan_rx_add_vid(adapter->netdev, 0);
#endif /* NETIF_F_HW_VLAN_CTAG_TX */
#else /* !HAVE_INT_NDO_VLAN_RX_ADD_VID */
	rnpgbevf_vlan_rx_add_vid(adapter->netdev, 0);
#endif /* HAVE_INT_NDO_VLAN_RX_ADD_VID */
	...
	for_each_set_bit(vid, adapter->active_vlans, VLAN_N_VID) {
		...
		rnpgbevf_vlan_rx_add_vid(adapter->netdev, htons(ETH_P_8021Q),
					 vid);
#else /* !NETIF_F_HW_VLAN_CTAG_TX */
		rnpgbevf_vlan_rx_add_vid(adapter->netdev, vid);
#endif /* NETIF_F_HW_VLAN_CTAG_TX */
#else /* !HAVE_INT_NDO_VLAN_RX_ADD_VID */
		rnpgbevf_vlan_rx_add_vid(adapter->netdev, vid);
#endif /* HAVE_INT_NDO_VLAN_RX_ADD_VID */
...
static u32 rnpgbevf_fix_features(struct net_device *netdev, u32 features)
#else
...
#ifdef HAVE_RHEL6_NET_DEVICE_OPS_EXT
static int rnpgbevf_set_features(struct net_device *netdev, u32 features)
#else
...
#ifdef FIX_VEB_BUG
static void rnpgbevf_force_src_mac(struct sk_buff *skb,
				   struct net_device *netdev)
{
	u8 *data = skb->data;
	bool ret = false;
					    ...
					    bool tx_padding)
{
	...
	if (!(tx_ring->ring_flags & RNPVF_RING_VEB_MULTI_FIX))
		rnpgbevf_force_src_mac(skb, tx_ring->netdev);
#endif
	...
	if (adapter->flags & RNPVF_FLAG_PF_SET_VLAN) {
		...
		// add a vlan, and set ignore valid
		__vlan_hwaccel_put_tag(skb, htons(ETH_P_8021Q), adapter->vf_vlan);
...
rnpgbevf_get_stats64(struct net_device *netdev, struct rtnl_link_stats64 *stats)
#endif
{
	struct rnpgbevf_adapter *adapter = netdev_priv(netdev);
	int i;
...
/**
 * rnpgbevf_get_stats - Get System Network Statistics
 * @netdev: network interface device structure
 *
 * Returns the address of the device statistics structure.
 * The statistics are actually updated from the timer callback.
 **/
static struct net_device_stats *rnpgbevf_get_stats(struct net_device *netdev)
{
	struct rnpgbevf_adapter *adapter = netdev_priv(netdev);
...
static const struct net_device_ops rnpgbevf_netdev_ops = {
...
#else
	.ndo_get_stats = rnpgbevf_get_stats,
#endif
	...
	.extended.ndo_change_mtu = rnpgbevf_change_mtu,
#else
...
static const struct net_device_ops_ext rnpgbevf_netdev_ops_ext = {
...
#ifdef HAVE_RHEL6_NET_DEVICE_OPS_EXT
	set_netdev_ops_ext(dev, &rnpgbevf_netdev_ops_ext);
#endif /* HAVE_RHEL6_NET_DEVICE_OPS_EXT */
#else /* HAVE_NET_DEVICE_OPS */
	dev->open = &rnpgbevf_open;
	dev->stop = &rnpgbevf_close;
	dev->hard_start_xmit = &rnpgbevf_xmit_frame;
#ifdef HAVE_SET_RX_MODE
	dev->set_rx_mode = &rnpgbevf_set_rx_mode;
#endif
	dev->set_multicast_list = &rnpgbevf_set_rx_mode;
	dev->set_mac_address = &rnpgbevf_set_mac;
	dev->change_mtu = &rnpgbevf_change_mtu;
#ifdef HAVE_TX_TIMEOUT
	dev->tx_timeout = &rnpgbevf_tx_timeout;
#endif
#if defined(NETIF_F_HW_VLAN_TX) || defined(NETIF_F_HW_VLAN_CTAG_TX)
	dev->vlan_rx_add_vid = &rnpgbevf_vlan_rx_add_vid;
	dev->vlan_rx_kill_vid = &rnpgbevf_vlan_rx_kill_vid;
#endif
...
#endif
	netdev->features |= rnpgbevf_tso_features(hw);
...
#ifdef DEBUG
/**
 * rnpgbevf_get_hw_dev_name - return device name string
 * used by hardware layer to print debugging information
 **/
char *rnpgbevf_get_hw_dev_name(struct rnpgbevf_hw *hw)
{
