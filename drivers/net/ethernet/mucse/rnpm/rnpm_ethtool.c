			     ...
			     struct ethtool_drvinfo *drvinfo)
{
	...
	strncpy(drvinfo->driver, rnpm_driver_name,
	...
	if (adapter->pf_adapter->priv_flags &
		...
		if ((hw->speed == SPEED_1000) ||
			...
			if (hw->supported_link &
				...
				if (hw->supported_link &
				    RNPM_SFP_MODE_10G_LR)
					hw->phy_type =
						PHY_TYPE_10G_BASE_LR;
				if (hw->supported_link &
				    RNPM_SFP_MODE_10G_SR)
					hw->phy_type =
						PHY_TYPE_10G_BASE_SR;
				if (hw->supported_link &
				    RNPM_SFP_MODE_10G_LRM)
					hw->phy_type =
						PHY_TYPE_10G_BASE_LR;
				if (hw->supported_link &
	...
	if (hw->phy.media_type == rnpm_media_type_fiber) {
		if (!adapter->sfp.mod_abs) {
			if (adapter->media_availble) {
				...
				if (err) {
					  rnpm_err("%s get lane stat err \n", adapter->netdev->name);
					  return err;
				}

				if (!adapter->sfp.mod_abs) {
					hw->phy.media_type = rnpm_media_type_unknown;
					pr_debug("%s hw->phy.media_type = %d adapter->sfp.mod_abs = %d\n",
					adapter->netdev->name, hw->phy.media_type, adapter->sfp.mod_abs);
				}
			} else {
				hw->phy.media_type = rnpm_media_type_unknown;
			}
			     ...
			     struct ethtool_cmd *ecmd)
{
	...
	rnpm_logd(
		LOG_ETHTOOL,
		"%s %s set link: speed=%d port=%d duplex=%d autoneg=%d\n",
		__func__, netdev->name, ecmd->speed, ecmd->port,
		ecmd->duplex, ecmd->autoneg);
	rnpm_logd(LOG_ETHTOOL, "phy_address=%d\n", ecmd->phy_address);
	 ...
	 * duplex is forced.
	 */
	if (ecmd->eth_tp_mdix_ctrl && hw->is_sgmii) {
		if (hw->phy.media_type != rnpm_media_type_copper)
			return -EOPNOTSUPP;
	...
	if ((advertising_link_speed | hw->supported_link) !=
	    hw->supported_link)
		return -EOPNOTSUPP;
	...
	if (change ||
	    (hw->phy.autoneg_advertised != advertising_link_speed)) {
		while (test_and_set_bit(__RNPM_IN_SFP_INIT,
					&adapter->state)) {
			timeout--;
			if (!timeout)
				return -EBUSY;
		...
		hw->mac.autotry_restart = true;
		err = hw->mac.ops.setup_link(hw, advertising_link_speed,
					     true);
		if (err) {
			e_info(probe, "setup link failed with code %d\n",
			       err);
...
#endif /* HAVE_ETHTOOL_EXTENDED_RINGPARAMS */
{
	...
	if (new_rx_count != adapter->rx_ring_item_count) {
		...
		for (i = 0; i < adapter->num_rx_queues; i++) {
			...
			err = rnpm_setup_rx_resources(&temp_ring[i],
						      adapter);
			if (err) {
				while (i) {
					i--;
...
#define SHORT_STATS
			...
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_tx_clean_desc", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_tx_poll_count", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
			...
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_send_bytes_to_hw", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_todo_update", i, dma_ch);
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_send_done_bytes", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
			...
			sprintf(p, "queue%u_dma%u_rx_driver_drop_packets",
				i, dma_ch);
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_rx_rsc", i, dma_ch);
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_rx_rsc_flush", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
			...
			sprintf(p, "queue%u_dma%u_rx_rm_vlan_packets", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_alloc_rx_page", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
			...
			sprintf(p, "queue%u_dma%u_rx_equal_count", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
			sprintf(p, "queue%u_dma%u_rx_poll_packets", i,
				dma_ch);
			p += ETH_GSTRING_LEN;
...
static int rnpm_priv_status_update(struct rnpm_adapter *adapter)
{
	...
	if (priv & RNPM_PRIV_FLAG_MAC_LOOPBACK) {
		...
		pf_adapter->priv_flags &=
			(~RNPM_PRIV_FLAG_SWITCH_LOOPBACK);
	...
	if (priv_flags & RNPM_FORCE_SPEED_ABLITY) {
		if (adapter->hw.max_speed_1g == 1) {
			...
			adapter->pf_adapter->force_10g_1g_speed_ablity =
				false;
			rnpm_err(
				"%s: max speed is 1G canot set force_speed_ablity priv-flags  !\n",
				adapter->netdev->name);
		} else {
			adapter->pf_adapter->priv_flags |=
				RNPM_PRIV_FLAG_FORCE_SPEED_ABLIY;
				   ...
				   struct ethtool_stats *stats, u64 *data)
{
	...
	for (i = 0; i < RNPM_GLOBAL_STATS_LEN; i++) {
		p = (char *)net_stats +
		    rnpm_gstrings_net_stats[i].stat_offset;
	...
	for (j = 0; j < RNPM_HWSTRINGS_STATS_LEN; j++, i++) {
		...
		data[i] = (rnpm_hwstrings_stats[j].sizeof_stat ==
			   sizeof(u64)) ?
				  *(u64 *)p :
				  *(u32 *)p;
	}
...
#ifdef HAVE_ETHTOOL_GET_RXNFC_VOID_RULE_LOCS
		ret = rnpm_get_ethtool_fdir_all(adapter, cmd,
						(u32 *)rule_locs);
#else
 ...
 */
static int
rnpm_set_rx_ntuple(struct net_device __always_unused *dev,
...
static int rnpm_get_rxfh(struct net_device *netdev,
...
#if defined(KYLIN_V4_ETHTOOL_FIX_BOND) || !defined(ETHTOOL_GLINKSETTINGS)
...
#ifdef ETHTOOL_COALESCE_MAX_FRAMES_IRQ
				     | ETHTOOL_COALESCE_MAX_FRAMES_IRQ
#endif
#ifdef ETHTOOL_COALESCE_MAX_FRAMES
