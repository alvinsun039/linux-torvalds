...
#ifdef HAVE_STRUCT_PTP_SYSTEM_TIMESTAMP
...
#define adapter_struct rnpm_adapter
#define adapter_q_vector rnpm_q_vector
...
#define CONFIG_RNPM_DISABLE_PACKET_SPLIT
#endif
...
#define NETIF_F_ALL_FCOE \
	(NETIF_F_FCOE_CRC | NETIF_F_FCOE_MTU | NETIF_F_FSO)
#endif
...
#define ETHTOOL_SRXCSUM \
	0x00000015 /* Set RX hw csum enable (ethtool_value) */
#endif
#ifndef ETHTOOL_GTXCSUM
#define ETHTOOL_GTXCSUM \
	0x00000016 /* Get TX hw csum enable (ethtool_value) */
#endif
#ifndef ETHTOOL_STXCSUM
#define ETHTOOL_STXCSUM \
	0x00000017 /* Set TX hw csum enable (ethtool_value) */
#endif
#ifndef ETHTOOL_GSG
#define ETHTOOL_GSG \
...
static inline int _kc_test_and_set_bit(int nr,
				       volatile unsigned long *addr)
{
	unsigned long mask = BIT_MASK(nr);
	unsigned long *p = ((unsigned long *)addr) + BIT_WORD(nr);
...
#define hlist_for_each_entry_safe(pos, n, head, member)                \
	for (n = NULL, pos = hlist_entry_safe((head)->first,           \
					      typeof(*(pos)), member); \
...
#define snprintf(buf, size, fmt, args...) \
	_kc_snprintf(buf, size, fmt, ##args)
...
unsigned long _kc_find_next_bit(const unsigned long *addr,
...
#define dma_pool_create(name, dev, size, align, allocation)       \
	pci_pool_create((name), to_pci_dev(dev), (size), (align), \
			(allocation))
#endif /* < 2.6.3 */
...
#define scnprintf(buf, size, fmt, args...) \
	_kc_scnprintf(buf, size, fmt, ##args)
#endif /* < 2.6.4 */
...
#endif
static inline unsigned _kc_compare_ether_addr(const u8 *addr1,
					      const u8 *addr2)
{
...
#define compare_ether_addr(addr1, addr2) \
	_kc_compare_ether_addr(addr1, addr2)
#endif /* < 2.6.15 */
...
_kc_request_irq(unsigned int irq, new_handler_t handler,
...
#if (!(RHEL_RELEASE_CODE && \
       RHEL_RELEASE_CODE > RHEL_RELEASE_VERSION(5, 5)))
...
static inline int __kc_skb_cow_head(struct sk_buff *skb,
...
#define device_set_wakeup_enable(dev, val)                           \
	do {                                                         \
		u16 pmc = 0;                                         \
		int pm = pci_find_capability(adapter->pdev,          \
					     PCI_CAP_ID_PM);         \
		if (pm) {                                            \
...
#define PCI_EXP_DEVCTL_BCR_FLR \
	0x8000 /* Bridge Configuration Retry / FLR */
...
#define skb_tx_hash(n, s) \
	___kc_skb_tx_hash((n), (s), (n)->real_num_tx_queues)
#undef CONFIG_FCOE
#undef CONFIG_FCOE_MODULE
...
#else /* 2.6.0 => 2.6.32 */
static inline int
_kc_pm_runtime_get_sync(struct device __always_unused *dev)
{
	return 1;
}
...
#else /* 2.6.0 => 2.6.34 */
static inline bool
_kc_pm_runtime_suspended(struct device __always_unused *dev)
{
	return false;
}
...
#define __netdev_alloc_skb_ip_align(d, l, _g) \
	netdev_alloc_skb_ip_align(d, l)
...
static inline void *_kc_dma_zalloc_coherent(struct device *dev,
					    size_t size,
...
#undef rnpm_get_netdev_tc_txq
#define rnpm_get_netdev_tc_txq(dev, tc) \
 ...
 */
static inline struct workqueue_struct *__attribute__((format(printf, 3,
							     4)))
...
static inline int __kc_ipv6_skip_exthdr(const struct sk_buff *skb,
...
#define ipv6_skip_exthdr(a, b, c, d) \
...
int __kc_pcie_capability_read_dword(struct pci_dev *dev, int pos,
				    u32 *val);
...
#define mod_delayed_work(wq, dwork, delay) \
	__mod_delayed_work(wq, dwork, delay)
...
#ifdef CONFIG_XPS
int __kc_netif_set_xps_queue(struct net_device *, const struct cpumask *,
			     u16);
...
#else
static inline int
__kc_pci_vfs_assigned(struct pci_dev __always_unused *dev)
{
	return 0;
}
#endif
...
static inline struct sk_buff *
__kc__vlan_hwaccel_put_tag(struct sk_buff *skb, u16 vlan_tci)
{
...
#define pcie_get_minimum_link(_p, _s, _w) \
	__kc_pcie_get_minimum_link(_p, _s, _w)
#endif

#if (RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(6, 7))
int _kc_pci_wait_for_pending_transaction(struct pci_dev *dev);
#define pci_wait_for_pending_transaction \
	_kc_pci_wait_for_pending_transaction
#endif /* <RHEL6.7 */
...
#define dma_set_mask_and_coherent(_p, _m) \
	__kc_dma_set_mask_and_coherent(_p, _m)
...
#define HAVE_VXLAN_CHECKS
     ...
     UBUNTU_VERSION_CODE >= UBUNTU_VERSION(3, 13, 0, 24))
#define HAVE_NDO_SELECT_QUEUE_ACCEL_FALLBACK
#else
...
#define ipv6_find_hdr(a, b, c, d, e) \
	__kc_ipv6_find_hdr((a), (b), (c), (d), (e))
...
static inline int
__kc_dev_uc_sync(struct net_device __maybe_unused *dev,
					      ...
					      const unsigned char *))
{
	...
	return __kc_dev_addr_sync_dev(&dev->uc_list, &dev->uc_count, dev,
				      sync, unsync);
#else
	return 0;
#endif
						...
						const unsigned char *))
{
	...
	__kc_dev_addr_unsync_dev(&dev->uc_list, &dev->uc_count, dev,
				 unsync);
...
static inline int
__kc_dev_mc_sync(struct net_device __maybe_unused *dev,
		 int __maybe_unused (*sync)(struct net_device *,
					    const unsigned char *),
		 int __maybe_unused (*unsync)(struct net_device *,
					      const unsigned char *))
...
#elif defined(HAVE_SET_RX_MODE)
	return __kc_dev_addr_sync_dev(&dev->mc_list, &dev->mc_count, dev,
				      sync, unsync);
#else
	return 0;
#endif
...
#else /* NETDEV_HW_ADDR_T_MULTICAST */
	__kc_dev_addr_unsync_dev(&dev->mc_list, &dev->mc_count, dev,
				 unsync);
...
void __kc_skb_complete_tx_timestamp(
	struct sk_buff *skb, struct skb_shared_hwtstamps *hwtstamps);
...
#else
unsigned int __kc_eth_get_headlen(unsigned char *data,
				  unsigned int max_len);
#endif /* !RHEL >= 8.2 */
...
#define NETDEV_RSS_KEY_LEN (13 * 4)
	 ...
	 RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(7, 0)) || \
	(RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(7, 2)))))
#define netdev_rss_key_fill(buffer, len) \
	__kc_netdev_rss_key_fill(buffer, len)
#endif /* RHEL_RELEASE_CODE */
...
#else /* < 4.0 */
#define HAVE_DDP_PROFILE_UPLOAD_SUPPORT
#define HAVE_CACHE_INFO_SUPPORT
#endif /* < 4.0 */
...
#endif
static inline void __kc_timecounter_adjtime(struct timecounter *tc,
					    s64 delta)
{
	tc->nsec += delta;
}
...
#if (UBUNTU_VERSION_CODE && \
     UBUNTU_VERSION_CODE < UBUNTU_VERSION(4, 8, 0, 0))
...
static inline dma_addr_t __kc_dma_map_page_attrs(
	struct device *dev, struct page *page, size_t offset, size_t size,
	enum dma_data_direction dir, unsigned long __always_unused attrs)
{
...
#define ethtool_link_ksettings_add_link_mode(ptr, name, mode)          \
	(*((ptr)->link_modes.name) |=                                  \
	 (typeof(*((ptr)->link_modes.name)))ETHTOOL_LINK_CONVERT(name, \
								 mode))
...
/* KylinOS desktop 4.4.131 kernerl ethtool support glinksettings， but not support for bond function */
...
_kc_array_index_mask_nospec(unsigned long index, unsigned long size)
{
	/*
	...
	return ~(long)(index | (size - 1UL - index)) >>
	       (BITS_PER_LONG - 1);
}
...
#define array_index_nospec(index, size)                      \
	({                                                   \
		...
		typeof(size) _s = (size);                    \
		unsigned long _mask =                        \
			_kc_array_index_mask_nospec(_i, _s); \
...
#define HAVE_ETHTOOL_PHY_STATISTICS
...
#define bitmap_zalloc(nbits, flags) \
	bitmap_alloc(nbits, ((flags) | __GFP_ZERO))
...
static inline bool
__kc_napi_if_scheduled_mark_missed(struct napi_struct *n)
{
...
#ifndef HAVE_STRUCT_PTP_SYSTEM_TIMESTAMP
...
#define eth_get_headlen(dev, data, len) \
	__kc_eth_get_headlen(dev, data, len)
#endif /* !eth_get_headlen */
#endif /* !NO_ETH_GET_HEADLEN */
#endif /* !RHEL >= 8.2 */
...
#define flow_block_cb_setup_simple(f, driver_list, cb, cb_ident, cb_priv, \
				   ingress_only)                          \
	_kc_flow_block_cb_setup_simple(f, driver_list, cb, cb_ident,      \
...
#define pci_get_dsn(dev) _kc_pci_get_dsn(dev)
	...
	!(RHEL_RELEASE_CODE &&                                    \
	  (RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(8, 3)))
#define pci_aer_clear_nonfatal_status \
...
#if (OPENEULER_VERSION_CODE == OPENEULER_VERSION(2203, 3))
