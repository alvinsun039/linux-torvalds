 ...
 * to trigger activation on one define
 */
 ...
 *       to avoid compatibility problems with early 2.6 kernels
 ...
 * declare it as zero on older kernels so that if it gets or geted in
...
#ifdef NEED_LINK_MODE_ZERO
static inline void linkmode_zero(unsigned long *dst)
{
#ifdef __ETHTOOL_LINK_MODE_MASK_NBITS
        bitmap_zero(dst, __ETHTOOL_LINK_MODE_MASK_NBITS);
#else
        bitmap_zero(dst, SUPPORTED_1000baseT_Full);
#endif
}
#endif
#ifdef NEED_LINK_MODE_SET_BIT
static inline void linkmode_set_bit(int nr, volatile unsigned long *addr)
{
        __set_bit(nr, addr);
}
#endif
#ifdef NEED_LINK_MODE_TEST_BIT
 ...
 * APIs as seamlessly as possible.
 *
