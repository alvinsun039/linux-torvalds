...
#define DRV_VERSION     __stringify(1.2.2-kylin)
...
char ngbe_driver_name[] = KBUILD_MODNAME;
...
char ngbe_firmware_version[NGBE_FW_VER_SIZE] = "N/A";
...
void ngbe_set_rx_mode(struct net_device *netdev)
{
	...
	if (flags & IFF_PROMISC)
		xcast_mode = NGBE_XCAST_MODE_PROMISC;
	else if (flags & IFF_ALLMULTI)
		xcast_mode = NGBE_XCAST_MODE_ALLMULTI;
	else if (flags & (IFF_BROADCAST | IFF_MULTICAST))
		xcast_mode = NGBE_XCAST_MODE_MULTI;
	else
		xcast_mode = NGBE_XCAST_MODE_NONE;
...
void ngbe_up_complete(struct ngbevf_adapter *adapter)
{
	...
	hw->mac.get_link_status = 1;
	spin_lock_bh(&adapter->mbx_lock);
	/* get link status when up for external phy*/
	hw->mac.ops.get_pf_linkstatus(hw);
	spin_unlock_bh(&adapter->mbx_lock);
...
void ngbe_reset(struct ngbevf_adapter *adapter)
{
	...
	if (err)
		DPRINTK(PROBE, DEBUG, "reset function failed, err=%d\n", err);
	else
				  ...
				  int rxr_count, int rxr_idx)
{
	...
	/* initialize NAPI */
	netif_napi_add(adapter->netdev, &q_vector->napi, ngbe_poll);
...
int ngbe_set_mac(struct net_device *netdev, void *p)
{
	...
	err = TCALL(hw, mac.ops.set_rar, 0, (u8 *)addr->sa_data, 0, 1);
...
#endif
	/*STAG_FILTER and CTAG_FILTER may make txgbevf in centos9.4 vlan dump*/
	/*#if 0
