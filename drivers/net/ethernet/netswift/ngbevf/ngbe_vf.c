...
s32 ngbe_init_ops_vf(struct ngbevf_hw *hw)
{
	...
	hw->mac.ops.get_fw_version = ngbe_get_fw_version;
	hw->mac.ops.get_pf_linkstatus = ngbe_get_pf_link_status;
	...
	hw->mac.ops.get_link_state = ngbe_get_link_state_vf;
...
s32 ngbe_update_xcast_mode(struct ngbevf_hw *hw, int xcast_mode)
{
	...
	case ngbe_mbox_api_12:
		if(xcast_mode > NGBE_XCAST_MODE_ALLMULTI)
...
/**
 * ngbe_get_link_state_vf - Get VF link state from PF
 * @hw: pointer to the HW structure
...
s32 ngbe_get_link_state_vf(struct ngbevf_hw *hw, bool *link_state)
{
	...
	msgbuf[0] = NGBE_VF_GET_LINK_STATE;
	msgbuf[1] = 0x0;
	...
	if (err || (msgbuf[0] & NGBE_VT_MSGTYPE_NACK)) {
...
s32 ngbe_get_pf_link_status(struct ngbevf_hw *hw)
{
	...
	msgbuf[0] = NGBE_VF_GET_LINK_STATUS;
	msgbuf[1] = 0x0;
	err = mbx->ops.write_posted(hw, msgbuf, 2, 0);
	if (err)
		return err;
	...
	adapter->pf_link_up = links_reg & NGBE_VXSTATUS_UP;
	if (!adapter->pf_link_up) {
		adapter->pf_speed = 0;
		return 0;
	}

	if (NGBE_PFLINK_SPEED(links_reg) == 0) {
		adapter->pf_link_up = false;
		return 0;
	}

	adapter->pf_speed = NGBE_PFLINK_SPEED(links_reg);
...
s32 ngbe_get_fw_version(struct ngbevf_hw *hw)
{
	...
	u32 msgbuf[2];
	s32 err;
	msgbuf[0] = NGBE_VF_GET_FW_VERSION;
	msgbuf[1] = 0x0;
	err = mbx->ops.write_posted(hw, msgbuf, 2, 0);
	if (err)
		return err;
	...
	if (err || (msgbuf[0] & NGBE_VT_MSGTYPE_NACK)) {
		...
		snprintf(ngbe_firmware_version, NGBE_FW_VER_SIZE, "0x%08x", msgbuf[1]);
		err = 0;
	}
	return err;
}
...
static int ngbe_notify_vf_link_status(struct ngbevf_hw *hw, u32 *msgbuf)
{
...
static int ngbe_pf_ping_vf(struct ngbevf_hw *hw, u32 *msgbuf)
{
	struct ngbevf_adapter *adapter = hw->back;
	s32 err = 0;
	u32 in_msg = msgbuf[0];
...
static int  ngbe_rcv_msg_from_pf(struct ngbevf_hw *hw)
{
	...
	retval = ngbe_read_mbx(hw, msgbuf, mbx_size, 0);
	...
	case NGBE_NOFITY_VF_LINK_STATUS:
		if (msgbuf[0] & NGBE_VT_MSGTYPE_NRN) 
			adapter->pf_running = false;
		else
			adapter->pf_running = true;
		retval = ngbe_notify_vf_link_status(hw, msgbuf);
		break;
	default:
		retval = ngbe_pf_ping_vf(hw, msgbuf);
		break;
	}
			    ...
			    bool *link_up, bool autoneg_wait_to_complete)
{
	...
	if (!ngbe_check_for_msg(hw, 0))
		if(ngbe_rcv_msg_from_pf(hw)) {
			err = -1;
			goto out;
		}
