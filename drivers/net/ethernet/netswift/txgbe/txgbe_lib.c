...
static bool txgbe_set_dcb_vmdq_queues(struct txgbe_adapter *adapter)
{
	...
	adapter->queues_per_pool = tcs;/*maybe same to num_rx_queues_per_pool*/
...
#if IS_ENABLED(CONFIG_FCOE)
	if (adapter->flags & TXGBE_FLAG_FCOE_ENABLED) {
		struct txgbe_ring_feature *fcoe;
		...
		if (fcoe_i) {
			...
			fcoe->indices = 1;
			fcoe->offset = txgbe_fcoe_get_tc(adapter);
		} else {
...
static bool txgbe_set_dcb_queues(struct txgbe_adapter *adapter)
{
	 ...
	 */
	if (adapter->flags & TXGBE_FLAG_FCOE_ENABLED) {
		u8 tc = txgbe_fcoe_get_tc(adapter);
				...
				unsigned int rxr_count, unsigned int rxr_idx)
{
	...
	atomic_set(&q_vector->state, TXGBE_QV_STATE_DISABLE);
	...
	while (rxr_count) {
		...
		if (adapter->flags & TXGBE_FLAG_FCOE_ENABLED) {
			...
			if ((rxr_idx >= f->offset) &&
				...
				set_bit(__TXGBE_RX_FCOE, &ring->state);
			}
		}
#endif /* CONFIG_FCOE */
