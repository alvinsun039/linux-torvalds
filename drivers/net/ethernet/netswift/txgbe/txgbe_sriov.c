...
static int txgbe_vf_backup(struct txgbe_adapter *adapter, u16 vf)
{
	...
	for (i = 0; i < 16; i++) {
		vfinfo->b4_buf[i] = 
			txgbe_rd32(TXGBE_BA4_ADDR(vfinfo, i * 4));
	}
#endif
	return 0;
}
...
#if 0
	for (i = 0; i < 16; i++) {
		txgbe_wr32(TXGBE_BA4_ADDR(vfinfo, i * 4),
			   vfinfo->b4_buf[i]);
	}
#endif
	return 0;
				   ...
				   u32 *msgbuf, u32 vf)
{
	...
	entries = min(entries, (u16)TXGBE_MAX_VF_MC_ENTRIES);
...
int txgbe_ndo_set_vf_bw(struct net_device *netdev,
			...
			int max_tx_rate)
#else
int txgbe_ndo_set_vf_bw(struct net_device *netdev, int vf, int max_tx_rate)
