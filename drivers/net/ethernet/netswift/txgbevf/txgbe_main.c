...
#define DRV_VERSION     __stringify(1.3.2-kylin)
		...
		struct xdp_buff __maybe_unused *xdp)
{
	...
	case XDP_TX:
		...
		break;
	default:
#ifdef HAVE_XDP_ACTION_NEW_WARN
...
void txgbe_reset(struct txgbe_adapter *adapter)
{
	...
	if (err)
		DPRINTK(PROBE, DEBUG, "reset function\n");
	else
				  ...
				  int rxr_count, int rxr_idx)
{
	...
	netif_napi_add(adapter->netdev, &q_vector->napi, txgbe_poll);
#else
...
void txgbe_update_stats(struct txgbe_adapter *adapter)
{
	...
	spin_unlock(&adapter->PCount_lock);
