...
s32 txgbe_init_ops_vf(struct txgbe_hw *hw)
{
	...
	/* Link */
	hw->mac.ops.setup_link = txgbe_setup_mac_link_vf;
...
static void txgbe_virt_clr_reg(struct txgbe_hw *hw)
{
	...
	//clear: TXGBE_VXRSSRK(i)
	//clear: TXGBE_VXRETA(i)
...
/**
 *  txgbe_stop_adapter_vf - Generic stop Tx/Rx units
 *  @hw: pointer to hardware structure
 *
...
/**
 *  txgbe_update_xcast_mode - Update Multicast mode
 *  @hw: pointer to the HW structure
...
/**
 * txgbe_get_link_state_vf - Get VF link state from PF
 * @hw: pointer to the HW structure
