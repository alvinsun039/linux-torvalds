				    ...
				    size_t size, loff_t *ppos)
{
	...
	case TXGBE_FUNC_DUMP_RDESC: {
		...
		u32 r_idx = TXGBE_DATA_ARGS(txgbe_data_mode);
				     ...
				     size_t count, loff_t *ppos)
{
	...
	} else if (!strncmp(txgbe_dbg_reg_ops_buf, "write", 5)) {
		u32 reg, value;
		int cnt;
	...
	} else if (!strncmp(txgbe_dbg_reg_ops_buf, "read", 4)) {
		u32 reg, value;
		int cnt;
