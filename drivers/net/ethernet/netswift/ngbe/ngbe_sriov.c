...
static void ngbe_set_vf_rx_tx(struct ngbe_adapter *adapter, int vf);
				...
				unsigned int num_vfs)
{
	...
	num_vf_macvlans = hw->mac.num_rar_entries -
		(NGBE_MAX_PF_MACVLANS + 1 + num_vfs);
...
void ngbe_enable_sriov(struct ngbe_adapter *adapter)
{
	...
	if (!__ngbe_enable_sriov(adapter, num_vfs)) {
			    ...
			    u32 vf)
{
	...
	max_frs = rd32(hw, NGBE_PSR_MAX_SZ);
				  ...
				  u32 *msgbuf, u32 vf)
{
	...
	msgbuf[1] = (adapter->link_speed << 1) | adapter->link_up;
...
static int ngbe_vf_reset_msg(struct ngbe_adapter *adapter, u16 vf)
{
	...
	ngbe_set_vf_rx_tx(adapter, vf);

	pf_max_frame = dev->mtu + ETH_HLEN;

	if (pf_max_frame > ETH_FRAME_LEN)
		reg = (1 << vf_shift);
	wr32(hw, NGBE_RDM_VFRE_CLR, reg);
...
static int ngbe_get_vf_link_state(struct ngbe_adapter *adapter,
				  u32 *msgbuf, u32 vf)
{
...
static int ngbe_get_fw_version(struct ngbe_adapter *adapter,
						      u32 *msgbuf, u32 vf)
{
...
static int ngbe_rcv_msg_from_vf(struct ngbe_adapter *adapter, u16 vf)
{
	...
	case NGBE_VF_UPDATE_XCAST_MODE:
		...
		break;
	case NGBE_VF_GET_LINK_STATE:
		retval = ngbe_get_vf_link_state(adapter, msgbuf, vf);
		break;
	case NGBE_VF_GET_LINK_STATUS:
		retval = ngbe_get_vf_link_status(adapter, msgbuf, vf);
		break;
	case NGBE_VF_GET_FW_VERSION:
		retval = ngbe_get_fw_version(adapter, msgbuf, vf);
	case NGBE_VF_BACKUP:
		break;
	default:
...
void ngbe_msg_task(struct ngbe_adapter *adapter)
{
	...
	spin_lock_irqsave(&adapter->mbx_lock, flags);
	...
	spin_unlock_irqrestore(&adapter->mbx_lock, flags);
}
...
/**
 * nxgbe_set_all_vfs - update vfs queues
 * @adapter: Pointer to adapter struct
 *
 * Update setting transmit and receive queues for all vfs
 **/
void ngbe_set_all_vfs(struct ngbe_adapter *adapter)
{
	int i;

	for (i = 0 ; i < adapter->num_vfs; i++) {
		ngbe_set_vf_rx_tx(adapter, i);
	}
}
...
static int ngbe_pci_sriov_enable(struct pci_dev *dev, int num_vfs)
{
	...
	err = __ngbe_enable_sriov(adapter, num_vfs);
	if (err)
		goto err_out;
...
/**
 * ngbe_set_vf_rx_tx - Set VF rx tx
 * @adapter: Pointer to adapter struct
 * @vf: VF identifier
 *
...
static void ngbe_set_vf_rx_tx(struct ngbe_adapter *adapter, int vf)
{
	...
	reg_cur_tx = rd32(hw, NGBE_TDM_POOL_TE);
	reg_cur_rx = rd32(hw, NGBE_RDM_POOL_RE);
	...
	if (adapter->vfinfo[vf].link_enable) {
		...
		if (reg_cur_tx != reg_req_tx)
			wr32(hw, NGBE_TDM_POOL_TE, reg_req_tx);
		if (reg_cur_rx != reg_req_rx)
			wr32(hw, NGBE_RDM_POOL_RE, reg_req_rx);
	} else {
		reg_req_tx = 1 << vf_shift;
		reg_req_rx = 1 << vf_shift;
		/* Disable particular VF */
		if (reg_cur_tx & reg_req_tx)
			wr32(hw, NGBE_TDM_VFTE_CLR, reg_req_tx);
		if (reg_cur_rx & reg_req_rx)
			wr32(hw, NGBE_RDM_VFRE_CLR, reg_req_rx);
	}
}

/**
 * ngbe_set_vf_link_state - Set link state
 * @adapter: Pointer to adapter struct
 * @vf: VF identifier
 ...
 **/
void ngbe_set_vf_link_state(struct ngbe_adapter *adapter, int vf, int state)
{
...
/**
 * ngbe_ndo_set_vf_link_state - Set link state
 ...
 **/
int ngbe_ndo_set_vf_link_state(struct net_device *netdev, int vf, int state)
{
	struct ngbe_adapter *adapter = netdev_priv(netdev);
	int ret = 0;
	...
	case IFLA_VF_LINK_STATE_DISABLE:
		...
		ngbe_set_vf_link_state(adapter, vf, state);
		break;
	case IFLA_VF_LINK_STATE_AUTO:
		dev_info(pci_dev_to_dev(adapter->pdev),
