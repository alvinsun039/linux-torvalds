...
static void ngbe_add_uc_addr(struct ngbe_hw *hw, u8 *addr, u32 vmdq)
{
	...
	if (hw->addr_ctrl.rar_used_count < rar_entries) {
		...
		hw->mac.ops.set_rar(hw, rar, addr, vmdq,
			NGBE_PSR_MAC_SWC_AD_H_AV);
 ...
 *  advertised settings
 **/
static int ngbe_negotiate_fc(struct ngbe_hw *hw, u32 adv_reg, u32 lp_reg,
 ...
 **/
int ngbe_set_vfta(struct ngbe_hw *hw, u32 vlan, u32 vind,
			   bool vlan_on)
{
	int regindex;
	u32 bitindex;
				 ...
				 u32 length, u32 timeout, bool return_data)
{
	...
	if (hw->mac.ops.acquire_swfw_sync(hw, NGBE_MNG_SWFW_SYNC_SW_MB)
	    != 0) {
		return NGBE_ERR_SWFW_SYNC;
	}
...
static int ngbe_fmgr_cmd_op(struct ngbe_hw *hw, u32 cmd, u32 cmd_addr)
{
	...
	cmd_val = NGBE_SPI_CMD_CMD(cmd) |
		  NGBE_SPI_CMD_CLK(SPI_CLK_DIV) |
		  cmd_addr;
				...
				const u8 *data, u32 size)
{
	...
	if (!vpd_tend) {
		printk("failed to allocate memory for vpd\n");
		return -ENOMEM;
	}
	...
	ngbe_flash_write_dword(hw, MAC_ADDR0_WORD1_OFFSET_1G, (mac_addr0_dword1_t | 0x80000000));//lan0
	ngbe_flash_write_dword(hw, MAC_ADDR1_WORD0_OFFSET_1G, mac_addr1_dword0_t);
	ngbe_flash_write_dword(hw, MAC_ADDR1_WORD1_OFFSET_1G, (mac_addr1_dword1_t | 0x80000000));//lan1
	ngbe_flash_write_dword(hw, MAC_ADDR2_WORD0_OFFSET_1G, mac_addr2_dword0_t);
	ngbe_flash_write_dword(hw, MAC_ADDR2_WORD1_OFFSET_1G, (mac_addr2_dword1_t | 0x80000000));//lan2
	ngbe_flash_write_dword(hw, MAC_ADDR3_WORD0_OFFSET_1G, mac_addr3_dword0_t);
	ngbe_flash_write_dword(hw, MAC_ADDR3_WORD1_OFFSET_1G, (mac_addr3_dword1_t | 0x80000000));//lan3
	if (sn_is_str) {
		for (i = 0; i < 24; i++) {
...
**/
static int ngbe_get_copper_link_capabilities(struct ngbe_hw *hw,
						 u32 *speed,
						 bool *autoneg)
{
	int status = 0;
 ...
 **/
int ngbe_setup_mac_link(struct ngbe_hw *hw,
			       u32 speed,
			       bool autoneg_wait_to_complete)
{
	...
	status = hw->mac.ops.check_link(hw,
 ...
 **/
static int ngbe_setup_copper_link(struct ngbe_hw *hw,
					 u32 speed,
					 bool need_restart_AN)
{
	...
	if (!((hw->subsystem_device_id & OEM_MASK) == RGMII_FPGA)) {
		status = hw->phy.ops.setup_link(hw, speed,
							need_restart_AN);
	}
...
static int ngbe_reset_misc(struct ngbe_hw *hw)
{
	...
	wr32m(hw, NGBE_MIS_RST_ST,
		NGBE_MIS_RST_ST_RST_INIT, 0xA00);
...
void ngbe_do_lanrst(struct ngbe_hw *hw)
{
	struct ngbe_adapter *adapter = hw->back;
 ...
 **/
static int ngbe_read_ee_hostif_data(struct ngbe_hw *hw, u16 offset,
				   u16 *data)
{
	int status;
	struct ngbe_hic_read_shadow_ram buffer;
...
bool ngbe_get_ncsi_pin_status(struct ngbe_hw *hw)
{
	...
	if (status) {
		ERROR_REPORT1(NGBE_ERROR_INVALID_STATE,
						 "ngbe_get_ncsi_pin_status failed\n");
		return false;
	}
 ...
 *  Reads a 16 bit word from the EEPROM using the hostif.
 **/
int ngbe_read_ee_hostif(struct ngbe_hw *hw, u16 offset,
			      u16 *data)
{
	int status = 0;
 ...
 **/
static int ngbe_read_ee_hostif_data32(struct ngbe_hw *hw, u16 offset,
					u32 *data)
{
	int status;
	struct ngbe_hic_read_shadow_ram buffer;
 ...
 *  Reads a 32 bit word from the EEPROM using the hostif.
 **/
int ngbe_read_ee_hostif32(struct ngbe_hw *hw, u16 offset,
					   u32 *data)
{
	int status = 0;
 ...
 **/
static int ngbe_write_ee_hostif_data(struct ngbe_hw *hw, u16 offset,
				    u16 data)
{
	int status;
...
*  Write a 16 bit word to the EEPROM using the hostif.
**/
static int ngbe_write_ee_hostif_data32(struct ngbe_hw *hw, u16 offset,
						u32 data)
{
	int status;
	struct ngbe_hic_write_shadow_ram buffer;
 ...
 **/
static int ngbe_check_mac_link(struct ngbe_hw *hw, u32 *speed,
...
static int ngbe_check_mac_link_mdi(struct ngbe_hw *hw,
							u32 *speed,
							bool *link_up,
...
static int ngbe_check_mac_link_yt8521s(struct ngbe_hw *hw,
							u32 *speed,
							bool *link_up,
							bool link_up_wait_to_complete)
{
...
static int ngbe_check_mac_link_ex(struct ngbe_hw *hw,
				  u32 *speed,
				  bool *link_up,
				  bool link_up_wait_to_complete)
{
	...
	if (link_up_wait_to_complete) {
		...
		if (hw->phy.phydev_type == ngbe_phy_jl)
			...
			ngbe_phy_read_reg_exphy(hw, RTL8211F_MII_BMSR, 0, &value);
		if (value & 0x4)
			*link_up = true;
		else
			*link_up = false;
...
void ngbe_init_ops(struct ngbe_hw *hw)
{
	...
	if (hw->phy.type == ngbe_phy_m88e1512 ||
		...
		phy->ops.get_lp_adv_pause = ngbe_phy_get_lp_advertised_pause_rtl821x;
		phy->ops.set_adv_pause = ngbe_phy_set_pause_advertisement_rtl821x;
	}
}
