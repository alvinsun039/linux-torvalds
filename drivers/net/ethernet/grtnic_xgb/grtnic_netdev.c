...
//	u16 temp;
	...
	if (wol_support) {
		...
		grtnic_PhyWrite(adapter->netdev, phy_addr, 0x1E, 0xA00A);	//wol cfg
		grtnic_PhyWrite(adapter->netdev, phy_addr, 0x1F, 0x004B);	//PME_N+WOL EN+PLUSE+168ms
	}
}
...
static void grtnic_configure(struct grtnic_adapter *adapter)
{
	...
	if ((card_type == board_902T_GRT_FF) && (hw->revision == 0)) {	//FF902T old version
		...
		grtnic_PhyWrite(netdev, phy_addr, 0x1F, 0x00); //page 0
		grtnic_PhyWrite(netdev, phy_addr, 0x0D, 0x07); //1 is phy add, 0d is MACR reg, 7 is device address
		grtnic_PhyWrite(netdev, phy_addr, 0x0E, 0x3C); //1 is phy add, 0e is MAADR reg, 0x3C is reg address
		grtnic_PhyWrite(netdev, phy_addr, 0x0D, 0x4007); //1 is phy add, 0d is MACR reg, 4007 get data from device add 7 & reg 14
		grtnic_PhyWrite(netdev, phy_addr, 0x0E, 0x00); // EEEAR
	}
	...
	dev_info(dev, "add=%02x:%02x:%02x:%02x:%02x:%02x\n", netdev->dev_addr[5], netdev->dev_addr[4], netdev->dev_addr[3], netdev->dev_addr[2],
...
static void grtnic_up_complete(struct grtnic_adapter *adapter)
{
	...
	if (hw->is_qsgmii) {	//FF904T bypass (use qsgmii phy)
		...
		grtnic_PhyWrite(netdev, phy_addr, 0x1F, 0x00); //写通用扩展寄存器 0xa000 为 0x0,选择 UTP 空间
		phy_addr = phy_addr + adapter->func; //更改utp的地址为对应口的地址
	}

	if (adapter->type == 1) {	//copper
		grtnic_PhyRead(netdev, phy_addr, 0x00, &temp); //prtad_reg
		grtnic_PhyWrite(netdev, phy_addr, 0x00, (temp & ~PHY_POWER_DOWN) | PHY_RESET); //rst phy
	} else {
