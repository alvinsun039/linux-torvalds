...
void grtnic_SetPause(struct net_device *netdev, u8 flowctl)
{
	...
	dev_info(dev, "RegFc = %08x, flowctl=%x\n", RegFc, flowctl);
...
static s32 grtnic_get_i2c_ack(struct grtnic_hw *hw)
{
  ...
  /* Poll for ACK.  Note that ACK in I2C spec is transition from 1 to 0 */
	for (i = 0; i < timeout; i++) {
		i2cctl = GRTNIC_READ_REG(hw, I2CCTL, 0);
	...
	if (ack) {
		DEBUGFUNC("I2C ack was not received.\n");
		status = GRTNIC_ERR_I2C;
	}
...
fail:
		...
		if (retry < max_retry)
			DEBUGFUNC("I2C byte read error - Retrying.\n");
		else
			DEBUGFUNC("I2C byte read error.\n");
		retry++;
