...
//		version = strtol(ipxe_ver_s,NULL,0);
	...
	netdev_info(netdev, "Found %s image File!!! ", (image_type == 1) ? "pxe" : (image_type == 2) ? "vpd" : "firmware");
...
static int grtnic_setup_loopback_test(struct grtnic_adapter *adapter)
{
	...
	if (adapter->ei->type == board_1002E_GRT_FF || adapter->ei->type == board_1005E_GRT_FX) {
		//enable loopback
		grtnic_SetPhyAddr(netdev, phy_addr, 0x01, 0x00); //prtad_devad_reg //mdio reg:1.0
		...
		if (hw->is_qsgmii) {	//FF904T bypass (use qsgmii phy)
			...
			if (adapter->type == 1) {	//copper
				/* Setup mac speed */
				grtnic_ResetRx(adapter->netdev);				//这里用adapter->netdev，因为之前的已经为了qsgmii更改了netdev
...
static void grtnic_loopback_cleanup(struct grtnic_adapter *adapter)
{
  ...
  /*Clear muliticast mode*/
	reg_data = grtnic_GetAdrsFilter(adapter->netdev); //这里用adapter->netdev，因为之前的已经为了qsgmii更改了netdev
	reg_data &= ~promisc_mode; //promisc
	grtnic_SetAdrsFilter(adapter->netdev, reg_data);
}
