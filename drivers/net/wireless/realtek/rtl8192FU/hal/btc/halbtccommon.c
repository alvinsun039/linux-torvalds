...
static const u32 coex_ver_date = 20220422;
static const u32 coex_ver = 0x2b;
...
exit:
	...
	/* TODO: For wifi/BT co-Rx, will enable when gain table is ready */
	/* btc->btc_write_scbd(btc, BTC_SCBD_BT_HILNA, TRUE); */
}
...
void rtw_btc_ex_display_coex_info(struct btc_coexist *btc)
{
	...
	if (is_bt_reply) {
		...
		if (coex_sta->bt_reg_vendor_dae == 0xffff) {
			...
			coex_sta->bt_reg_vendor_dae = val;
		}
