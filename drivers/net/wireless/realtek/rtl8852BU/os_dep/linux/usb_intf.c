...
static struct usb_device_id rtw_usb_id_tbl[] = {
	...
	{USB_DEVICE_AND_INTERFACE_INFO(0x3574, 0x6121, 0xff, 0xff, 0xff), .driver_info = RTL8852B}, /* Mandy add for sihaizhonglian 8832bu */
	{USB_DEVICE_AND_INTERFACE_INFO(0x3574, 0x6241, 0xff, 0xff, 0xff), .driver_info = RTL8852B}, /* Mandy add for sihaizhonglian 8852bu */
	...
	/*=== Realtek demoboard ===*/
	{USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0xC852, 0xff, 0xff, 0xff), .driver_info = RTL8852BT},
	...
	{USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0x8842, 0xff, 0xff, 0xff), .driver_info = RTL8852C},
#endif /* CONFIG_RTL8852C */

#ifdef CONFIG_RTL8852D
        /*=== Realtek demoboard ===*/
        {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0xD85A, 0xff, 0xff, 0xff), .driver_info = RTL8852D},
        {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0xD83A, 0xff, 0xff, 0xff), .driver_info = RTL8852D},
        {USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0xD85B, 0xff, 0xff, 0xff), .driver_info = RTL8852D},
	...
	for (i = 0; i < pusb_data->nr_endpoint; i++) {
		...
		if (phost_endp) {
			...
			if (RT_usb_endpoint_is_bulk_in(pendp_desc)) {
				if (pusb_data->RtNumInPipes == MAX_BULKIN_NUM) {
				...
				if (pusb_data->RtNumOutPipes == MAX_BULKOUT_NUM) {
...
static int rtw_dev_probe(struct usb_interface *pusb_intf, const struct usb_device_id *pdid)
{
	...
	RTW_INFO("+%s\n", __func__);
	mark_partner_supported_module("Realtek Semiconductor Corp.", THIS_MODULE);
