		   ...
		   enum phl_phy_idx phy_idx)
{
	...
	if (phy_idx == HW_PHY_0) {
		for (i = 0; i < 16; i++) {
			he_sigb->ehtsig_sigb_cr[i].address_0 = (u8)(n_sym_sigb_ch1_phy0[i] & 0xff);
			he_sigb->ehtsig_sigb_cr[i].address_1 = (u8)((n_sym_sigb_ch1_phy0[i] & 0xff00) >> 8);
			he_sigb->ehtsig_sigb_cr[i].address_2 = (u8)((n_sym_sigb_ch1_phy0[i] & 0xff0000) >> 16);
			he_sigb->ehtsig_sigb_cr[i].address_3 = (u8)((n_sym_sigb_ch1_phy0[i] & 0xff000000) >> 24);

			he_sigb->ehtsig_sigb_cr[i + 16].address_0 = (u8)(n_sym_sigb_ch2_phy0[i] & 0xff);
			he_sigb->ehtsig_sigb_cr[i + 16].address_1 = (u8)((n_sym_sigb_ch2_phy0[i] & 0xff00) >> 8);
			he_sigb->ehtsig_sigb_cr[i + 16].address_2 = (u8)((n_sym_sigb_ch2_phy0[i] & 0xff0000) >> 16);
			he_sigb->ehtsig_sigb_cr[i + 16].address_3 = (u8)((n_sym_sigb_ch2_phy0[i] & 0xff000000) >> 24);
		}
	}
		    ...
		    char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[2], "ppdu_type") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->ppdu_type = (u8)val[0];
	} else if (_os_strcmp(input[2], "stbc") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->stbc = (u8)val[0];
	} else if (_os_strcmp(input[2], "he_dcm_sigb") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->he_dcm_sigb = (u8)val[0];
	} else if (_os_strcmp(input[2], "he_mcs_sigb") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->he_mcs_sigb = (u8)val[0];
	} else if (_os_strcmp(input[2], "dbw") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->dbw = (u8)val[0];
	} else if (_os_strcmp(input[2], "gi") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->gi = (u8)val[0];
	} else if (_os_strcmp(input[2], "ltf_type") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->he_ltf_type = (u8)val[0];
	} else if (_os_strcmp(input[2], "mode") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->mode = (u8)val[0];
	} else if (_os_strcmp(input[2], "max_tx_time_0p4us") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->max_tx_time_0p4us = (u16)val[0];
	} else if (_os_strcmp(input[2], "n_user") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->n_user = (u8)val[0];
	} else if (_os_strcmp(input[2], "he_er_u106ru_en") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->he_er_u106ru_en = (u8)val[0];
	} else if (_os_strcmp(input[2], "tb_l_len") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->tb_l_len = (u16)val[0];
	} else if (_os_strcmp(input[2], "tb_ru_tot_sts_max") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->tb_ru_tot_sts_max = (u8)val[0];
	} else if (_os_strcmp(input[2], "tb_disam") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->tb_disam = (u8)val[0];
	} else if (_os_strcmp(input[2], "tb_ldpc_extra") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->tb_ldpc_extra = (u8)val[0];
	} else if (_os_strcmp(input[2], "tb_pre_fec_padding_factor") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->tb_pre_fec_padding_factor = (u8)val[0];
	} else if (_os_strcmp(input[2], "long_preamble_en") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->long_preamble_en = (bool)val[0];
	} else if (_os_strcmp(input[2], "cbw") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->cbw = (u8)val[0];
	} else if (_os_strcmp(input[2], "txsc") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->txsc = (u8)val[0];
	}else if (_os_strcmp(input[2], "punc_pattern") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->punc_pattern = (u8)val[0];
	}else if (_os_strcmp(input[2], "tb_mumimo_mode_en") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->tb_mumimo_mode_en = (u8)val[0];
	} else if (_os_strcmp(input[2], "txsb") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->txsb = (u8)val[0];
	} else if (_os_strcmp(input[2], "eht_mcs_sig") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->eht_mcs_sig = (u8)val[0];
	} else if (_os_strcmp(input[2], "user_idx") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		HALBB_SCAN(input[5], DCMD_DECIMAL, &val[1]);
		if (_os_strcmp(input[4], "nss") == 0) {
			in->usr[val[0]].nss = (u8)val[1];
		} else if (_os_strcmp(input[4], "fec") == 0) {
			in->usr[val[0]].fec = (u8)val[1];
		} else if (_os_strcmp(input[4], "payload") == 0 || _os_strcmp(input[4], "apep") == 0) {
			in->usr[val[0]].apep = val[1];
		} else if (_os_strcmp(input[4], "dcm") == 0) {
			in->usr[val[0]].dcm = (bool)val[1];
		} else if (_os_strcmp(input[4], "mcs") == 0) {
			in->usr[val[0]].mcs = (u8)val[1];
		} else if (_os_strcmp(input[4], "mpdu_len") == 0) {
			in->usr[val[0]].mpdu_len = (u16)val[1];
		} else if (_os_strcmp(input[4], "n_mpdu") == 0) {
			in->usr[val[0]].n_mpdu = (u16)val[1];
		} else if (_os_strcmp(input[4], "scrambler_seed") == 0) {
			in->usr[val[0]].scrambler_seed = (u8)val[1];
		} else if (_os_strcmp(input[4], "random_init_seed") == 0) {
			in->usr[val[0]].random_init_seed = (u8)val[1];
		} else if (_os_strcmp(input[4], "aid") == 0) {
			in->usr[val[0]].aid = (u16)val[1];
		} else if (_os_strcmp(input[4], "ru_alloc") == 0) {
			in->usr[val[0]].ru_alloc = (u8)val[1];
		} else if (_os_strcmp(input[4], "pwr_boost_db") == 0) {
			in->usr[val[0]].pwr_boost_db = (u8)val[1];
		}
	}
/*The following parameters are fixed in halbb_plcp_gen_init_7*/
#if 0
	else if (_os_strcmp(input[2], "bss_color") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->bss_color = (u8)val[0];
	} else if (_os_strcmp(input[2], "ht_l_len") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->ht_l_len = (u16)val[0];
	} else if (_os_strcmp(input[2], "ndp_en") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->ndp_en = (u8)val[0];
	} else if (_os_strcmp(input[2], "doppler") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->doppler = (u8)val[0];
	} else if (_os_strcmp(input[2], "nominal_t_pe") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->nominal_t_pe = (u8)val[0];
	} else if (_os_strcmp(input[2], "ness") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->ness = (u8)val[0];
	}
#endif
		    ...
		    char *output, u32 *_out_len)
{
	...
	BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
		    "tb_ru_tot_sts_max : %5d,                  tb_disam : %5d\n", in->tb_ru_tot_sts_max, in->tb_disam);
	BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
		    "    tb_ldpc_extra : %5d, tb_pre_fec_padding_factor : %5d\n", in->tb_ldpc_extra, in->tb_pre_fec_padding_factor);
	BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
		    "max_tx_time_0p4us : %5d,          long_preamble_en : %5d\n", in->max_tx_time_0p4us, in->long_preamble_en);
	BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
		    "     punc_pattern : %5d,         tb_mumimo_mode_en : %5d\n", in->punc_pattern, in->tb_mumimo_mode_en);
	BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
		    "      eht_mcs_sig : %5d\n", in->eht_mcs_sig);
/*The following parameters are fixed in halbb_plcp_gen_init_7*/
#if 0
...
void halbb_plcp_cmd_default(struct bb_info *bb, struct halbb_plcp_info *in,
		    struct usr_plcp_gen_in *user, char input[][16], u32 *_used,
		    char *output, u32 *_out_len)
{
	in->ppdu_type = 0; //4-bit: 0: bmode,1:Legacy,2:HT_MF,3:HT_GF,4:VHT,5:HE_SU,6:HE_ER_SU,7:HE_MU,8:HE_TB,9:EHT_SU,10:EHT_MU,11:EHT_RU,12:EHT_ERSU,13:EHT_TB
	in->n_user = 1;
	in->cbw = 0;
	in->dbw = 0; //3-bit: 0:BW20, 1:BW40, 2:BW80, 3:BW160/BW80+80, 4:BW320
	in->gi = 0;
	in->he_ltf_type = 0; // ltf_type : 1x, 2x, 4x
	in->mode = 0; //0:apep, 1:max_tx_time, 2:n_mpdu,mpdu_len, 3:tb_trigger_mode
	in->stbc = 0;
	in->he_dcm_sigb = 0;
	in->he_mcs_sigb = 0;
...
/*The following parameters are fixed in halbb_plcp_gen_init_7*/
	...
	in->usr[0].nss = 1;
	in->usr[0].fec = 0;
	in->usr[0].apep = 1000; // payload length
	in->usr[0].dcm = 0;
	in->usr[0].mcs = 0;
	in->usr[0].mpdu_len = 0;
	in->usr[0].n_mpdu = 0;
	in->usr[0].scrambler_seed = 104;
	in->usr[0].random_init_seed = 63;
	in->usr[0].aid = 0;
	in->usr[0].ru_alloc = 0;
		    ...
		    char input[][16], u32 *_used, char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], "set") == 0){
		...
		halbb_plcp_cmd_default(bb, in, in->usr, input, _used, output, _out_len);
