...
static int virtnet_get_rxfh(struct net_device *dev, u32 *indir, u8 *key, u8 *hfunc)
{
	...
	if (indir) {
		for (i = 0; i < vi->rss_indir_table_size; ++i)
			indir[i] = le16_to_cpu(vi->rss_hdr->indirection_table[i]);
	}

	if (key)
		memcpy(key, vi->rss_hash_key_data, vi->rss_key_size);
...
static int virtnet_set_rxfh(struct net_device *dev, const u32 *indir, const u8 *key, const u8 hfunc)
{
	...
	if (indir) {
		...
		for (i = 0; i < vi->rss_indir_table_size; ++i)
			vi->rss_hdr->indirection_table[i] = cpu_to_le16(indir[i]);
		update = true;
	}
	if (key) {
		...
		memcpy(vi->rss_hash_key_data, key, vi->rss_key_size);
		update = true;
	}
