					...
					const struct regulator_desc *desc)
{
	...
	if (constraints->settling_time_up && constraints->settling_time) {
		pr_warn("%pS: ambiguous configuration for settling time, ignoring 'regulator-settling-time-up-us'\n",
			np);
	...
	if (constraints->settling_time_down && constraints->settling_time) {
		pr_warn("%pS: ambiguous configuration for settling time, ignoring 'regulator-settling-time-down-us'\n",
			np);
	...
	if (!fwnode_property_read_u32(np, "regulator-initial-mode", &pval)) {
		if (desc && desc->of_map_mode) {
			...
			if (mode == REGULATOR_MODE_INVALID)
				pr_err("%pS: invalid mode %u\n", np, pval);
			else
				constraints->initial_mode = mode;
		} else {
			pr_warn("%pS: mapping for mode %d not defined\n",
				np, pval);
		}
	}
		...
		if (desc && desc->fwnode_map_mode) {
			...
			if (!ret) {
				for (i = 0; i < len; i++) {
					...
					if (mode == REGULATOR_MODE_INVALID)
						pr_err("%pS: invalid regulator-allowed-modes element %u\n",
							np, pval);
					else
			...
			pr_warn("%pS: mode mapping not defined\n", np);
		}
	}
		       ...
		       unsigned int num_matches)
{
	...
	fwnode_for_each_child_node(node, child) {
		...
		for (i = 0; i < num_matches; i++) {
			...
			if (!match->init_data) {
				dev_err(dev,
					"failed to parse Fwnode for regulator %pS\n",
					child);
					    ...
					    struct fwnode_handle **node)
{
	...
	if (!init_data) {
		dev_err(dev, "failed to parse Fwnode for regulator %pS\n", child);
		goto error;
	}
	...
	if (desc->fwnode_parse_cb) {
		...
		if (ret) {
			...
			dev_err(dev,
				"driver callback failed to parse Fwnode for regulator %pS\n",
				child);
			goto error;
		}
	}
 ...
 */
static int __maybe_unused is_supply_name(const char *name)
{
	int strs, i;
