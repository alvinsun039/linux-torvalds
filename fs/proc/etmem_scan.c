...
#ifdef CONFIG_ARM64
#define if_pmd_thp_or_huge(pmd) ({				\
	typeof(pmd) _pmd = (pmd);				\
	if_pmd_huge(_pmd) || pmd_trans_huge(_pmd);		\
})
#endif /* CONFIG_ARM64  */
...
#define set_restart_gpa(val, note) ({				\
	...
	pic->restart_gpa = _val;				\
	trace_printk("restart_gpa=%lx %luK	%s	%s %d\n",	\
		     _val, (pic->restart_gpa - old_val) >> 10,	\
...
enum proc_idle_page_type (*vm_handle_pte_hole)(unsigned long addr,
					       unsigned long next,
					       int depth,
...
static const struct kernel_param_ops walk_step_ops = {
	.set = set_walk_step,
	.get = param_get_uint,
};
...
static void u64_to_u8(u64 n, uint8_t *p)
{
	p += sizeof(u64) - 1;
...
static int pic_add_page(struct page_idle_ctrl *pic,
			unsigned long addr,
			unsigned long next,
			enum proc_idle_page_type page_type)
{
...
static enum proc_idle_page_type ept_huge_accessed(pmd_t *pmd, unsigned long addr,
						  unsigned long end)
{
			 ...
			 struct mm_walk *walk)
{
	...
	unsigned long next;
	enum proc_idle_page_type page_type;
	enum proc_idle_page_type pte_page_type;
	int err = 0;
			  ...
			  unsigned long end)
{
	...
	read_lock(&pic->kvm->mmu_lock);
	pgd = (pgd_t *)kvm->arch.mmu.pgt->pgd + pgd_index(addr) * PTRS_PER_PTE;
	read_unlock(&pic->kvm->mmu_lock);
...
{
	struct page_idle_ctrl *pic = walk->private;
	enum proc_idle_page_type pagetype;

	if (!vm_handle_pte_hole)
		return 0;
			    ...
			    size_t count, loff_t *ppos)
{
	...
	mm_walk_ops = kzalloc(sizeof(*mm_walk_ops), GFP_KERNEL);
	if (!mm_walk_ops) {
		kfree(pic);
		return -ENOMEM;
	}
